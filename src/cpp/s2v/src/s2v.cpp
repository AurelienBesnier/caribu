/*	S4 CALCULE LA REPARTITION DANS UNE GRILLE 3D DES SURFACES DES
 * TRIANGLES*/
/*	CONSTITUTIFS D'UNE MAQUETTE, AINSI QUE LA DISTRIBUTION D'ORIENTATION
 * DES*/
...
/*	nb1 : Dans la suite on parle indifferement d'inclinaison ou d'angle
 * zenital, ces*/
...
/* 	(espece= label/10**11) */
/*	nb3 :La structure peut etre de type periodique dans le plan
 * horizontal.*/
/* 	(typiquement une periode = un interrang). */
/*	Une maquette periodique est constituee de la repetition d'un motif
 * elementaire.*/
/*	Dans ce cas l'ensemble des triangles est analyse pour calculer les
 * variations*/
/*	moyennes a l'interieur du motif. Dans le plan horizontal, le motif est
 * divise en*/
...
/*	par des commandes parameter. Modifier ces lignes si necessaire pour
 * augmenter*/
...
/***************      Prototypes      ********************/
double
lectri(signed char&, char&, int&, long[], int&, Patch&, FILE*);
int
repart(Patch, char, int);
void
calcjp(Patch, int[3][3], char&);
void
classe(double, int&);
// BUG multi-specie - MCoct2010 : void affect(Patch,int*);
void
affect(Patch, int*, int);
double
proscal(double*, double*);
void
provec(double*, double*, double*);
void
norme(double*, double*);
void
normal(Patch&, double&, double&);
double
area(Patch& T);
// CF 2016 read number of optical species in opt file
void
lect_nopt(int* nopt, char* optname);
void
lect_po(Tabdyn<double, 3>& Tpo, int po, char* optname);
int
isid(char* name);

// Secured file close
void
Sfclose(FILE** fic, int line = 161); // HA nov 2003
...
s2v(int argc, char** argv)
{
        ...
        long i_att[NattMax];
        FILE *fpar = NULL, *fmlsail = NULL, *fsail = NULL, *ftri = NULL,
             *fsurf = NULL;
        ...
        if (argc > 1) { // by shared memory (called by caribu)
                ...
                if (isid(argv[1])) { // by seg
                        ...
                        assert((hSharedSeg = OpenFileMapping(
                                  FILE_MAP_ALL_ACCESS, FALSE, clef_alphanum)) !=
                               NULL);

                        lpSharedSeg = MapViewOfFile(hSharedSeg,
                                                    FILE_MAP_ALL_ACCESS,
                                                    0,
                                                    0,
                                                    SEGSIZE * sizeof(Patch));
                ...
                xpo.alloue(nje, njz, npo, 2);
                xpo.maj(0); // CF 2016: xpo a 4 dim now
        ...
        disti.alloue(nje, nji);
        disti.maj(0);
        dista.alloue(nje, nja);
        dista.maj(0);
        xlad.alloue(nje, njz);
        xlad.maj(0);
        xladi.alloue(nje, njz, nji);
        xladi.maj(0);
        xladia.alloue(nje, njz, nji, nja);
        xladia.maj(0);
        ...
        /* lecture d'un triangle, calcul de sa classe d'orientation et de la
           cellule de chaque sommet Si besoin estn subsidvison recursive du T en
           cas d'a cheval
        */
        ...
        do {
                if (segpar) {
                        ...
                        if (test == 1) {
                                ...
                                nbtri++;
                                // i_att[0] = label1/1000
                                je = (i_att[0] / 100000000) -
                                     1; // je : de 0 � nje (indice tableau C)
                                if (je + 1 > nje_reel)
                                        nje_reel = je + 1;
                                // Bug MC09 T.t=je+1;
                                ...
                                // \n",i_att[0],i_att[0]/1000); MC09
                                // i_att[0]=i_att[0]%1000;
                                i_att[0] = T.t = (opak > 0) ? 1 : 0;
                                // if(i_att[0]>0) T.t= -T.t;
                                //  i_att[0] = 1 => Transparent: feuille
                                //  i_att[0] = 0 => Opak: Tige
                                //  printf("> s2v: lectri() => id:%.0f,
                                //  opak=%ld,  i_att[0]=%ld,  je=%d,
                                //  T.t=%d\n",id, opak,i_att[0], je, T.t);
                        }
                } // else segpar

                // debug MC2011
                //   printf(">>>> Apres lcture fichier fort.51 nje = %d,
                //   nje_reel =%d\n", nje, nje_reel) ;
                if (test > -1) {
                        // fprintf(stderr,"label=%d, esp=%d, nbid=%d
                        // ,nbs=%d\n",i_att[0],je,natt,nsom);
                        if ((je < 0) && (je >= nje) && (natt <= 0) &&
                            (natt > NattMax) && (nsom != 3)) {
                                fprintf(stderr,
                                        "*** Incorect data format in fort.51 "
                                        "file -> break\n");
                                exit(-1);
                        }
                        ...
                        double incl, azi;
                        // for(i=0;i<3;i++)for(j=0;j<3;j++) printf("T(%d,%d) =
                        // %lf\n",i,j,T[i][j]);
                        ...
                        // printf("inc=%lf, azi=%lf => ji=%d,
                        // ja=%d\n",incl,azi,ji,ja);
                        // calcul recursif des coord discretes du T et du cas
                        // cheval printf("Repart called 1, je=%d\n", je);
                        // for(int q=0;q<3;q++)
                        // printf("Patch(%d,2)=%g\n",q,T.P[q][2]);
                        if (je >= 0) {
                                il = repart(T, 0, je);
                ...
                // printf("> dbg: juste avant while(!segpar && feof(ftri): cptr
                // = %d\n", cptr++);
        ...
        */
        if (nbtt < 1)
                ...
                fflush(stderr);
                double tmp;
                /* Inversion de l'ordre des boucles en mettant je en 1er pour
                   eviter les cas ou une couche ne contient pas une espece et
                   pour faire des calculs especes confondues - MC98 */
                int ii = 0;
                // bug mai 2011
                jx = jy = 0; // jy n'avait pas l'air d'etre init et il n'y a pas
                             // de bocle sur x et y
                for (jz = 0; jz < njz; jz++) {
                        for (je = 0; je < nje; je++) {
                                for (ji = 0; ji < nji; ji++) {
                                        ...
                                        //   xladi(je,  jz, ji)=ii++; //dbug mc
                                        //   2010
                                        // dbug: printf(">> xladi(jz=%d, je=%d,
                                        // ji=%d) =%.0lf\n",jz,je,ji, xladi(je,
                                        // jz,
                                        // ji));fflush(stdout);fflush(stderr);
                                } // for ji
                ...
                /* 	Calcul des distributions globales d'orientation des
                 * feuilles */
                for (je = 0; je < nje; je++) {
                        for (ja = 0; ja < nja; ja++) {
                                if (surft[je] > 0)
                ...
                /* 	Editions */
                Ferr << "=> Ecriture des resultats : (c|std)err, leafarea, "
                        "out.dang"
                     << '\n';
                // stdout
                if (nbz > 0)
                        Ferr << "Il y a eu " << nbz << " depassement en z+"
                             << '\n';
                if (nbzm > 0)
                        Ferr << "Il y a eu " << nbzm << " depassement en z-"
                             << 'n';
                ...
                for (je = 0; je < nje; je++) {
                        ...
                        // d'inclinaison :",je+1, surft[je],Stot,xlai[je]);
                        Ferr << "esp " << je + 1 << " : surfT=" << surft[je]
                             << " - Stot=" << Stot;
                ...
                // genere le fichier out.dang => entree de sailM pour calculer
                // la BRDF
                Ferr << "genere le fichier out.dang => entree de sailM pour "
                        "calculer la BRDF"
                     << '\n';
                ...
                for (je = 0; je < nje; je++) {
                        for (jz = 0; jz < njz; jz++) {
                                t += xlad(je, jz);
                                if (xlad(je, jz) >
                                    0.) { // -1000 MCoct2010 debug
                                        printf("\n %d  %d \t %7.6lf\t ",
                                               jz + 1,
                                               je + 1,
                                               (xlad(je, jz) / volume[jz]));
                                        for (ji = 0; ji < nji; ji++)
                                                printf("%7.6lf  ",
                                                       xladi(je, jz, ji));
                                }
                ...
                if (genopt) {
                        ...
                        for (po = 0; po < npo; po++) {
                                ...
                                for (jz = 0; jz < njz; jz++) {
                                        ...
                                        for (je = 0; je < nje; je++) {
                                                ...
                                                if (xlad(je, jz) > 0) {
                                                        ...
                                                        // po(je=%d/%d)=%g,
                                                        // Rfz=%g,
                                                        // Rft=%g\n",je,nje,xpo(je,0,0,jz,po,0),xpo(je,0,0,jz,po,0)*100./xx,
                                                        // Rf*100./x); printf("
                                                        // =>
                                                        // xx=%g,x=%g\n",xx,x);
                                                }
                                                // else printf(" pas de p.o. car
                                                // lai nul\n");
                                        } // for je
                                        if (x > 0) {
                                                Rf *= 100. / x;
                                ...
                                Sfclose(&fpar, __LINE__);
                                // Genere le fichier CROPCHAR degenere
                                // necessaire a MCsail
...
/**************************************************************************/
int
repart(Patch T, char level, int je)
{
        Patch exT, ssT;
        char acv;
        int jp[3][3];
        ...
        if (acv == 0) {
                ...
                if (level == LevelMax) {
                        // niveau subdiv max atteint => on stocke dans la cell
                        // de G
                        // printf("ACV mais levalMax: level = %d\n",(int)level);
                        for (i = 0; i < 3; i++)
                                G[i] =
                                  (exT.P[0][i] + exT.P[1][i] + exT.P[2][i]) /
                                  3.;
                        if (G[2] >= 0) {
                                jp[0][0] = (int)(G[0] / dx);
                                jp[0][1] = (int)(G[1] / dy);
...
/**************************************************************************/
void
calcjp(Patch T, int jp[3][3], char& acv)
{
        /* 	Calcul des positions en unites dx dy dz */
        /*	nb: le mode d'affectation est tel que la position (0,0,1) est
         * centree sur (0, 0, 0.5*bz(njz))*/
...
/***********************************************/
void
classe(double z, int& jz)
{
        /* 	Classement de la valeur z. Les classes sont definies par leur
         * borne */
        /* 	sup�rieures, le numero de classe est dans le sens des z
         * decroissants.
         */
...
affect(Patch T, int* jp, int je)
{
        ...
        /*  mise a jour du tableau xladia avec un triangle dont les 3 sommets
         * appar tiennent a une meme cellule
         */
        jz = jp[2];
        ...
        nbpatc++;
        // printf("surftri=%lf\n",surftri);
        // printf("affect() je=%d, jx=%d, jy=%d, jz=%d, ji=%d et
        // ja=%d\n",je,jx,jy,jz,ji,ja);
        //  printf("affect() je=%d, T.t=%d,
        ...
        if (genopt) {
                for (po = 0; po < npo; po++) {
                        ...
                        if (T.t > 0) { // Transparent
                                xpo(je, jz, po, 0) +=
                                  surftri * Tpo(po, je, 1); // Rf
                                xpo(je, jz, po, 1) +=
                                  surftri * Tpo(po, je, 2); // Tf
                                // printf("> affect(): cas transperentt: je=%d,
                                // jz=%d :: surftri=%.3g,
                                // Tpo(%d,%d,0)=%g\n",je,jz,surftri,po,T.t,Tpo(po,je,0));
                        } else { // Opak
                                printf(
                                  "> affect() :Cas opak: Tpo(%d,%d,0)=%g\n",
                                  po,
                                  T.t,
                                  Tpo(po, je, 0));
                                xpo(je, jz, po, 0) +=
                                  surftri * Tpo(po, je, 0); // Rt
                        }
                } // for po
        }         // if segpar
} // affect()
...
/********************************************/
void
provec(double* a, double* b, double* c)
{
        c[0] = a[1] * b[2] - a[2] * b[1];
        c[1] = -a[0] * b[2] + a[2] * b[0];
        c[2] = a[0] * b[1] - a[1] * b[0];
...
norme(double* x, double* xn)
{
        ...
        if (xnor < 1e-20) {
                fprintf(stderr,
                        "<!> s2v.cpp: norme() erreur: nombre=%g < 1e-20!! => "
                        "exit(-1)\n",
                        xnor);
                exit(-1);
        }
...
/***********************************************************************/
void
normal(Patch& T, double& zen, double& az)
{
        ...
        u = T.P[0][1] * p3p2[2] - T.P[0][2] * p3p2[1] + T.P[1][1] * T.P[2][2] -
            T.P[2][1] * T.P[1][2];
        v = -T.P[0][0] * p3p2[2] + T.P[0][2] * p3p2[0] - T.P[1][0] * T.P[2][2] +
            T.P[2][0] * T.P[1][2];
        w = T.P[0][0] * p3p2[1] - T.P[0][1] * p3p2[0] + T.P[1][0] * T.P[2][1] -
            T.P[2][0] * T.P[1][1];
        r2 = u * u + v * v;
        if (r2 == 0) {
                zen = 0;
