...
#define PAUSE(msg)                                                             \
        printf(msg);                                                           \
        printf("- Taper la touche Any");                                       \
        getchar();

// Cas de SAIL pur (Dsph==0)
void
Canopy::sail_pur(VEC** Cfar, double* Eclt, char* envname)
{
        int Nc;
        ...
        if (fenv == NULL) {
                fflush(stdout);
                Ferr << "<!> FF.C <init_NFF> Unable to open the file "
                     << envname << '\n';
                exit(4);
        }
        ...
        for (Ldiff.debut(); !Ldiff.finito(); Ldiff.suivant()) {
                ...
                if (layer == 0.0) {          // eg sol
                        ...
                        Ei[0] = layer * Tenv(il, 0) +
                                (1 - layer) * Tenv(il + 1, 0); // E-(z)
                        Ei[1] = layer * Tenv(il, 1) +
                                (1 - layer) * Tenv(il + 1, 1); // E+(z)
                }
                // printf("(%d,%d) layer=%lf, Ei[0]=%lf,
                // Ei[1]=%lf\n",(int)il,(int)(il+1),layer,Ei[0], Ei[1]);
                // Eclairement face sup
                E[0] = .5 * (1 + costl) * Ei[up] +
                       .5 * (1 - costl) *
                         Ei[down]; // Formules dans (Ross,1981) p.131
                ...
                if (!diffR->isopaque()) {
                  ...
                  // printf("costl=%lf => E[0]=%g,rho[0]=%g,
                // E[1]=%g\n",costl,E[0],rho[0], E[1]); printf("==>
                // Bsup(%ld)=%g,
...
colorie_triangle(void*,
                 ...
                 double,
                 double,
                 void (*f)(void*, int, int, void*));
void
colorie_capteur(REELLE** Zbuf,
                Punkt a,
                Punkt b,
                Punkt c,
                int Tx,
...
pt2pkt(Point&, Punkt);
void
zproj(void*, int, int, void*);
void
zFF(void*, int, int, void*);

//+************ zproj()
void
zproj(void* Zprim, int i, int j, void* prim)
{
...
static int
addbox(BSP* box, reel dx, reel dy, Boxi* Tabox, int ind)
{
        int i;
        if (ind != 0) {
                if (box->nb_diffuseur() == 0)
                        return ind;
                for (i = 0; i < ind; i++)
                        if (box == Tabox[i].box && Tabox[i].inc[0] == dx &&
                            Tabox[i].inc[1] == dy)
                                // BUG : if(box==Tabox[i].box &&
                                // Tabox[ind].inc[0]==dx &&
                                // Tabox[ind].inc[1]==dy)
                                return ind;
        }
  ...
  int nbsim)
{
        ...
        Point G, T;
        reel move[2];
        double mid, S[3], dGS[3], d2env = denv * denv, ff,
                                  dER2; // attention aux tests entre d et d^2
        ...
        for (Ldiff.debut(); !Ldiff.finito(); Ldiff.suivant()) {
                ...
                for (i = 0; i < 3; i++) {
                        ...
                        if ((inc[i] != 0 || (infty && mesh.nb_voxel(i) == 1 &&
                                             i != 2 && move[i] != 0)) &&
                            fabs(G[i] - S[i]) > denv) {
                                inc[i] = 0;
                                if (i != 2)
                ...
                if (inc[0] != 0 ||
                    (infty && mesh.nb_voxel(0) == 1 && move[0] != 0)) {
                        nb_box = addbox(mesh(Gi[0] + inc[0], Gi[1], Gi[2]),
                                        move[0],
                                        0,
                                        Tabox,
                                        nb_box);
                ...
                if (inc[1] != 0 ||
                    (infty && mesh.nb_voxel(1) == 1 && move[1] != 0)) {
                        nb_box = addbox(mesh(Gi[0], Gi[1] + inc[1], Gi[2]),
                                        0,
                                        move[1],
                                        Tabox,
                                        nb_box);
                ...
                if (inc[2] != 0) {
                        nb_box = addbox(mesh(Gi[0], Gi[1], Gi[2] + inc[2]),
                                        0,
                                        0,
                                        Tabox,
                                        nb_box);
                ...
                if (axe[0] == 0) {
                        ...
                        if ((dGS[0] + dGS[1]) < d2env)
                                nb_box = addbox(
                                  mesh(Gi[0] + inc[0], Gi[1] + inc[1], Gi[2]),
                                  move[0],
                                  move[1],
                                  Tabox,
                                  nb_box);
                ...
                if (axe[1] == 0) {
                        ...
                        if ((dGS[1] + dGS[2]) < d2env)
                                nb_box = addbox(
                                  mesh(Gi[0], Gi[1] + inc[1], Gi[2] + inc[2]),
                                  0,
                                  move[1],
                                  Tabox,
                                  nb_box);
                ...
                if (axe[2] == 0) {
                        ...
                        if ((dGS[2] + dGS[0]) < d2env)
                                nb_box = addbox(
                                  mesh(Gi[0] + inc[0], Gi[1], Gi[2] + inc[2]),
                                  move[0],
                                  0,
                                  Tabox,
                                  nb_box);
                ...
                if (diag == 0) {
                        ...
                        if ((dGS[0] + dGS[1] + dGS[2]) < d2env) {
                                // printf("mesh(%d+%d, %d+%d, %d+%d),
                                // move(%lf,%lf), mesh[%d,%d,%d]
                                // \n",Gi[0],inc[0],Gi[1],inc[1],Gi[2],inc[2],move[0],move[1],
                                // mesh.nb_voxel(0), mesh.nb_voxel(1),
                                // mesh.nb_voxel(2));fflush(stdout);
                                nb_box = addbox(mesh(Gi[0] + inc[0],
                                                     Gi[1] + inc[1],
                                                     Gi[2] + inc[2]),
                                                move[0],
                                                move[1],
                ...
                for (b = 0; b < nb_box; b++) { // loop sur les boites dans denv
                        box = Tabox[b].box;
                        // printf("Boite no %d/%d  = dx = %f, dy =
                        // %f\n",b,nb_box,Tabox[b].inc[0],Tabox[b].inc[1]);
                        //  Ferr << __FILE__ " : "<< __LINE__ << '\n' ;
                        // Ferr << "Boite no "<<b<<'\n';
                        // loop sur les diffuseurs de la boite
                        for (box->Ldiff.debut(); !box->Ldiff.finito();
                             box->Ldiff.suivant()) {
                                // Ferr << __FILE__ " : "<< __LINE__ << '\n' ;
                                diffE = box->Ldiff.contenu();
                                if (diffE->isreal() && diffE != diffR)
                                        // si diffE n'est pas un capteur virtuel
                                        // et si pas cas Diagonale : FF=1
                                        if (!(sol &&
                                              diffE->primi().name() == 0)) {
                                                // interaction sol-sol (H sol
                                                // plan) cas du diffuseur patche
                                                // tmax=diffE->nb_patch();
                                                // for(t=0;t<tmax;t++) {
                                                // diffE->select_patch(t);

                                                nb_test++;
                                                // OLD si bias==false, test de
                                                // distance pas fait et triangle
                                                // traite OLDif(!(bias &&
                                                // G.dist2(T)> d2env)) {
                                                // si bias==false, test de
                                                // distance point-triangle si
                                                // bias==true , triangle teste
                                                // que si  G.dist2(T)>d2env est
                                                // faux
                                                T = diffE->centre();
                                                T[0] += Tabox[b].inc[0];
                                                T[1] += Tabox[b].inc[1];
                                                if (bias) {
                                                        ...
                                                        polystr[255],str[100];
                                                        Polygone *E; // Perte du
                                                        polymorphisme(a voir)

                                                        sprintf(polystr,"%d
                                                        ",diffE->primi().nb_sommet());
                                                        for(signed char
                                                        p=0;p<diffE->primi().nb_sommet();p++){
                                                          sprintf(str," %g %g %g
                                                        ",
                                                        ...
                                                        //printf("polystr =
                                                        %s\n",polystr); E = new
                                                        Polygone(polystr,1,NULL,NULL,false);
                                                        T=E->centre();
                                                        dER2=E->distance2_point(G);
                                                        delete E;
                                                        */
                                                        // calcul un sous
                                                        // estimateur de la dist
                                                        // au triangle pour
                                                        // selectionner les
                                                        // projetes
                                                        Point I;
                                                        ...
                                                        // printf("denv=%g,
                                                        // select=%d\n",denv,(int)
                                                        // select);//I.show();diffE->show();
                                                }
                                                if (select) {
                                                        Vecteur dir(G, T);
                                                        if (dir.prod_scalaire(
                                                              diffE
                                                                ->normal()) !=
                                                            0) {
                                                                nb_ff++;
                ...
                if (verbose > 3) {
                        tps.Stop();
                        cout << ++nop << " : proj ortho-sph de " << nb_ff
                             << " T en " << tps << endl;
                        // Bug MC2006: ++nop est fait deux fois!!
                        //     Ferr <<++nop<<" : proj ortho-sph de "<<nb_ff<<" T
                        //     en "<<tps<<'\n';
                        Ferr << nop << " : proj ortho-sph de " << nb_ff
                             << " T en " << tps << '\n';
                }
...
Canopy::calc_FF(SPMAT* FF)
{
        Ferr << "Sorry, but your version desn't have the Schoeder Form Factor "
                "library"
                " \n => contact chelle@bcgn.grignon.inra.fr"
             << (char)7 << "\n";
} // Canopy::calc_FF()
#endif
...
void
Canopy::projplan(Vecteur& visee, bool infty, double* Bo)
{
        ...
        if (verbose > 2)
                printf("%c => projplan() DEBUT res. %d x %d\n", 7, Timg, Timg);
        ...
        if (fabs(visee[2] + 1.0) < 1e-6) {
                ...
                Ecran[3][0] = vmax[0];
                Ecran[3][1] = vmax[1];
                Ecran[0][2] = Ecran[1][2] = Ecran[2][2] = Ecran[3][2] =
                  vmax[2] + 1;
                du = vmax[0] - vmin[0];
                dv = vmax[1] - vmin[1];
                u = visee;
                ...
                double M[2][2], tmp, maxi[2] = { -999999.9, -999999.9 },
                                     mini[2] = { 999999.9, 999999.9 },
                                     dz = vmax[2] - vmin[2];
                ...
                for (i = 0; i < 4; i++) {
                        ...
                        for (j = 0; j < 2; j++) {
                                Ecran[i][j] =
                                  M[j][0] * tmp + M[j][1] * Ecran[i][1];
                        }
                        // Ecran[i].show();
                }
                // redressement de l'ecran tq ortho a visee (Ecran 0 et 3 : u
                // min)
                dv = tmp = (maxi[0] - mini[0]) * u.prod_scalaire(w);
                // printf("mini[0] = %g - mini[1] = %g ; maxi[0]= %g - maxi[1] =
                // %g\n",mini[0],mini[1],maxi[0], maxi[1]); printf(" translation
                // de d de la base Ecran : d = %g\n",tmp);
        ...
        /* validations
          ...
          // Bounding Box
          fprintf(fcan,"p 1 1 4 %g %g %g  %g %g %g  %g %g %g  %g %g
         %g\n",vmin[0],vmin[1],vmin[2], vmax[0],vmin[1],vmin[2],
         vmax[0],vmin[1],vmax[2], vmin[0],vmin[1],vmax[2]); fprintf(fcan,"p 1 2
         4 %g %g %g  %g %g %g  %g %g %g  %g %g %g\n",vmax[0],vmin[1],vmin[2],
         vmax[0],vmax[1],vmin[2], vmax[0],vmax[1],vmax[2],
         vmax[0],vmin[1],vmax[2]); fprintf(fcan,"p 1 3 4 %g %g %g  %g %g %g  %g
         %g %g  %g %g %g\n",vmax[0],vmax[1],vmin[2], vmin[0],vmax[1],vmin[2],
         vmin[0],vmax[1],vmax[2], vmax[0],vmax[1],vmax[2]); fprintf(fcan,"p 1 4
         4 %g %g %g  %g %g %g  %g %g %g  %g %g %g\n",vmin[0],vmax[1],vmin[2],
         vmin[0],vmin[1],vmin[2], vmin[0],vmin[1],vmax[2],
         vmin[0],vmax[1],vmax[2]); fprintf(fcan,"p 1 5 4 %g %g %g  %g %g %g  %g
         %g %g  %g %g %g\n",vmin[0],vmin[1],vmax[2], vmax[0],vmin[1],vmax[2],
         vmax[0],vmax[1],vmax[2], vmin[0],vmax[1],vmax[2]);
          //Ecran de projection
         fprintf(fcan,"p 1 0 4 %g %g %g  %g %g %g  %g %g %g  %g %g
         %g\n",Ecran[0][0],Ecran[0][1],Ecran[0][2],
         Ecran[1][0],Ecran[1][1],Ecran[1][2],
         Ecran[2][0],Ecran[2][1],Ecran[2][2],
         Ecran[3][0],Ecran[3][1],Ecran[3][2]); fclose(fcan);
          }
        */
        ...
        // int comptr;
        // comptr=0;
        for (Ldiff.debut(); (!Ldiff.finito()) && Ldiff.contenu()->isreal();
             Ldiff.suivant()) {
                // Ferr<<"* diff no. "<<++comptr<<endl;
                pdiff = Ldiff.contenu();
                pastoutvu = false;
                // cout <<"Canopy[projplan] primitive =
                // "<<pdiff->primi().name()<<endl; cout <<"Canopy[projplan]
                // P{Re} = ";Ecran[i+1].show();
                /*if( (Ecran[i+1][2]<0.0) || pastoutvu){  // Prim  PARTIELLEMENT
                  pas vue if( Ecran[i+1][2]<0.0) { pastoutvu=true;
                  //cout <<"Camera[calc_visi] Z neg\n";
                  }
                  }
                  else{
                  */
                // cout <<"Camera[projplan] Pp{Rimage}["<<i<<"]  =
                // ";Pp[i].show();
                switch (pdiff->acv) {
                        case 0:
                                acv_fin = 0;
                ...
                if (!pastoutvu)
                        for (acv_idx = 0; acv_idx <= acv_fin; acv_idx++) {
                                ...
                                for (i = 0; i < 3; i++) { // Cas des triangles
                                        ...
                                        Ecran[i + 1] -= SvE;
                                        Ecran[i + 1] =
                                          Ecran[i + 1].chgt_base(v, w, u);
                                        Pp[i][0] = Ecran[i + 1][0];
                                        Pp[i][1] = Ecran[i + 1][1];
                                        Pp[i][2] = Ecran[i + 1][2]; // distZ;
                                } // for triangle
                                // Tri sommets tq Pp[i][1]<<Pp[j][1]<<Pp[k][1]
                                // ie A[1] < B[1] < C[1]
                                j = (Pp[1][1] > Pp[2][1]) ? 1
                                                          : 2; // calc intermed
                                k = (Pp[0][1] > Pp[j][1])
                                      ? 0
                                      : j; // indice max pour coord y
                                i = (k + 1) % 3;
                                j = (i + 1) % 3;
                                i = (Pp[i][1] < Pp[j][1])
                                      ? i
                                      : j; // indice min pour coord y
                                j = 3 - i - k;
                                //       cout<<" (i,j,k) = "<<i<<j<<k<<endl;
                                if ((i != k) &&
                                    !((A[0] == B[0]) && (B[0] == C[0])) &&
                                    !((A[1] == B[1]) && (B[1] == C[1]))) {
                                        // Pts A,B,Cpas  alignes selon les axes
                                        // Xou Y tri Ok
                                        up = down = false;
                                        if (A[1] == B[1]) // up
                                        {
                                                ...
                                                } // else cas quelconque, ni up
                                                  // , ni down
                                        }
                                        //     cout<<"2 (i,j,k) =
                                        //     "<<i<<j<<k<<endl;
                                        // rappel syntaxe colorie_triangle()
                                        // void colorie_triangle( void *
                                        // tria,void *Zprim, double **Zbuf,Punkt
                                        // a,Punkt b,Punkt c, int Tx, int
                                        // Ty,double tx, double ty,void
                                        // (*f)(void * Zprim, int i, int j,
                                        // void* tria)){
        ...
        if (infty && visee[2] > -1 + 1e-6) {
                int** roofi;
                double cdist; // cste de distance ne depend que de l'inclinaison
                              // de la visee
                roofi = new int*[4];
                for (i = 0; i < 4; i++) {
                        roofi[i] = new int[2];
                ...
                infinitise(
                  (void***)Zprim, Zbuf, cdist, roofi, Timg, Timg, false);
                for (i = 0; i < 4; i++)
                        delete[] roofi[i];
                delete[] roofi;
        } // if infty
        ...
        if (nbcell > 0) {
                ...
                for (; !Ldiff.finito(); Ldiff.suivant()) {
                        pdiff = Ldiff.contenu();
                        if (pdiff->isreal()) {
                                Ferr << "<!> Attention triangle reel dans la "
                                        "liste capteur virtuel!!\n";
                                continue;
                        }
                        // Ferr<<"\t
                        // prodscal="<<visee.prod_scalaire(pdiff->normal())<<endl;
                        if (visee.prod_scalaire(pdiff->normal()) <
                            0) { // capteur bien vu par dessus
                                nbpix = 0;
                                switch (pdiff->acv) {
                                        case 0:
                                                acv_fin = 0;
                                        ...
                                        if ((i != k) &&
                                            ...
                                            !((A[1] == B[1]) &&
                                              (B[1] == C[1]))) {
                                                ...
                                                // rappel syntaxe
                                                // colorie_capteur()
                                                // void colorie_capteur(  double
                                                // **Zbuf,Punkt a,Punkt b,Punkt
                                                // c, int Tx, int Ty,double tx,
                                                // double ty,double nbpix)){

                                                if (up) {
                                                        pt2pkt(A, a);
                                                        pt2pkt(B, b);
                                                        pt2pkt(C, c);
                                                        colorie_capteur(Zbuf,
                                                                        c,
                                                                        a,
                                                                        b,
                                                                        Timg,
        ...
        double cocnomen, alt;
        int nummer;
        double cocmax = 0, cocmin = 99999999999., altmin = 99999999.,
               altmax = 0;
        // calcul de l'eclairage direct
        if (verbose > 1)
                printf(
                  "projplan() : du=%lf - dv=%lf =>  Apix= %lf\n", du, dv, Apix);
        for (i = 0; i < Timg; i++)
                for (j = 0; j < Timg; j++) {
                        pdiff = Zprim[i][j];
                        if (0) {
                                cocnomen = (pdiff == NULL)
                                             ? 0
                                             : pdiff->primi().name() / 1e7;
                                alt = (pdiff == NULL) ? 0 : 100 * Zbuf[i][j];
                                cocmax =
                                  (cocnomen > cocmax) ? cocnomen : cocmax;
                                cocmin =
                                  (cocnomen < cocmin) ? cocnomen : cocmin;
        ...
        %d %d\n255\n",altmin,altmax,Timg,Timg); fprintf(fprim,"P5\n# Radioxity -
        Image des labels \n# BitMin=%lf - BitMax=%lf\n %d
        %d\n255\n",cocmin,cocmax,Timg,Timg); altmax+=(altmax-altmin)/10.;
...
colorie_triangle(void* tria,
                 ...
                 void (*f)(void*, int, int, void*))
{
        double penteL, penteR, xL, xR, zL, yrel, vab, vac, vbc, z,
          dz; // L Left, R  Right
        ...
        // cout<<"\t A : ";a.show();cout<<"\t B : "; b.show();cout<<"\t C : ";
        // c.show(); cout<<"\t deby = "<<deby  <<" - finy = "<<finy<<" - yrel =
        // "<<yrel<<endl; cout<<"\t xL   = "<<xL    <<" - xR   = "<<xR<<"
        // -zL="<<zL<<endl; cout<<"\tpenteL= "<<penteL<<" -penteR= "<<penteR<<"
        // - sens = "<<(int) sens<<" - vab= "<<vab<<" - vbc = "<<vbc<<endl;
        ...
        for (j = deby; j <= finy; j++) {
                ...
                // cout<<"LOOP\t xL= "<<xL    <<" - xR= "<<xR<<
                // "(iL,iR)="<<iL<<", "<<iR<<" -j= "<<j<<" -zL="<<zL<< endl;
                if (iL <= iR) {
                        // dz= yrel*vbc/(xR-xL);
                        z = zL;
                        for (i = iL; i <= iR; i++) {
                                // cout<<"Capteur[colorie-triangle] [i][j] =
                                // "<<i<<", "<<j<<endl; cout <<"Camera
                                // [colorie_triangle][i][j] ("<<i<<","<<j<<") z
                                // = "<<z<<"- Z = "<<Zbuf[i][j]<<endl;
                                if (z < Zbuf[i][j]) {
                                        // cout <<"Camera
                                        // [colorie_triangle][i][j]
                                        // ("<<i<<","<<j<<") z = "<<z<<"- Z =
                                        // "<<Zbuf[i][j]<<endl;
                                        Zbuf[i][j] = (REELLE)z;
                                        // ndi[i][j]=idif;
...
void
Canopy::data3d(int tx, int ty, Vecteur& visee, bool infty, long int**& Zno)
{
        int i, j, k, l, img_surf;
        ...
        // Bounding Box
        fprintf(fcan,
                "p 1 1 4 %g %g %g  %g %g %g  %g %g %g  %g %g %g\n",
                vmin[0],
                vmin[1],
                vmin[2],
                vmax[0],
                ...
                vmax[2]);
        fprintf(fcan,
                "p 1 2 4 %g %g %g  %g %g %g  %g %g %g  %g %g %g\n",
                vmax[0],
                vmin[1],
                vmin[2],
                vmax[0],
                ...
                vmax[2]);
        fprintf(fcan,
                "p 1 3 4 %g %g %g  %g %g %g  %g %g %g  %g %g %g\n",
                vmax[0],
                vmax[1],
                vmin[2],
                vmin[0],
                ...
                vmax[2]);
        fprintf(fcan,
                "p 1 4 4 %g %g %g  %g %g %g  %g %g %g  %g %g %g\n",
                vmin[0],
                vmax[1],
                vmin[2],
                vmin[0],
                ...
                vmax[2]);
        fprintf(fcan,
                "p 1 5 4 %g %g %g  %g %g %g  %g %g %g  %g %g %g\n",
                vmin[0],
                vmin[1],
                vmax[2],
                vmax[0],
        ...
        // Ecran de projection
        fprintf(fcan,
                "p 1 0 4 %g %g %g  %g %g %g  %g %g %g  %g %g %g\n",
                Ecran[0][0],
                Ecran[0][1],
                Ecran[0][2],
                Ecran[1][0],
        ...
        for (Ldiff.debut(); !Ldiff.finito(); Ldiff.suivant()) {
                pdiff = Ldiff.contenu();
                pastoutvu = false;
                // cout <<"Canopy[data3D] primitive =
                // "<<pdiff->primi().name()<<endl; cout <<"Canopy[data3d] P{Re}
                // = ";Ecran[i+1].show();
                /*if( (Ecran[i+1][2]<0.0) || pastoutvu){  // Prim  PARTIELLEMENT
                  pas vue if( Ecran[i+1][2]<0.0) { pastoutvu=true;
                  //cout <<"Camera[calc_visi] Z neg\n";
        ...
        for (i = 0; i < imgZ.taille(0); i++)
                for (j = 0; j < imgZ.taille(1); j++) {
                        ...
                        alt = (pdiff == NULL) ? 0 : 100 * Zbuf[i][j];
                        imgZ.maj(
                          imgZ.taille(0) - 1 - i, imgZ.taille(1) - 1 - j, alt);
                        if (pdiff != NULL) {
                                pdiff->active(visee);
                                Zno[i][j] = pdiff->num();
                                pdiff->active(0);
...
Canopy::calc_FF_Bfar(SPMAT* FF,
                     ...
                     int nbsim)
{
        int Tx = 100, Ty = 100 Tabdyn<double, 2> Zbuf(Tx, Ty);
        ...
        for (Ldiff.debut(); !Ldiff.finito(); Ldiff.suivant()) {
                ...
                /*     if(pdiff->primi().name()==0) //maj visi sol
                       ...
                       */
                if (pdiff->primi().name() == 0)
                        pdiff->primi().show("Camera[calc_visi] ");
                pastoutvu = false;
                // cout <<"Camera[calc_visi] primitive =
                // "<<pdiff->primi().name()<<endl;
                for (i = 0; i < 3; i++) { // Cas des triangles
                        P[i] = pdiff->primi()[i];
                        P[i] -= SvE;
                        P[i] = P[i].chgt_base(u, v, w);
                        cout << "Camera[calc_visi] P{Re} = ";
                        P[i].show();
                        if ((P[i][2] < 0.0) ||
                            pastoutvu) // Prim  PARTIELLEMENT pas vue
                                if (P[i][2] < 0.0) { //
                                        pastoutvu = true;
                                        // cout <<"Camera[calc_visi] Z neg\n";
                                } else {
                                        ...
                                        (*pdiff)++;
                                        (*pdiff)--;
                                        // cout <<"Camera[calc_visi] P vu mais
                                        // pastoutvu actif!\n";
                                }
                        else {
                                ...
                                Pp[i][2] = 0.0; // P[i][2];
                                // cout <<"Camera[calc_visi] Pp{Re}["<<i<<"]  =
                                // ";Pp[i].show();
                                //  Chgt d'origine E->O (coinBG)
                                //  distZ=Pp[i].dist2(P[i]);
                                Pp[i] -= EvI;
                                Pp[i][2] = P[i][2]; // distZ;
                                cout << "Camera[calc_visi] Pp{Rimage}[" << i
                                     << "]  = ";
                                Pp[i].show();
                        } // else P[i][2]<0.0) || pastoutvu)
                }         // for points triangle
                if (!pastoutvu) {
                        // Tri sommets tq Pp[i][1]<<Pp[j][1]<<Pp[k][1] ie A[1] <
                        // B[1] < C[1]
                     ...
                     int nbsim)
{
        ...
        BSP *box, *Tabox[8];
        Point G, T;
        double mid, S[3], dGS[3], d2env = denv * denv,
                                  ff; // attention aux tests entre d et d^2
        ...
        for (Ldiff.debut(); !Ldiff.finito(); Ldiff.suivant()) {
                ...
                if (inc[0] != 0) {
                        nb_box = addbox(
                          mesh(Gi[0] + inc[0], Gi[1], Gi[2]), Tabox, nb_box);
                        axe[0]--;
                        axe[2]--;
                        diag--;
                }
                if (inc[1] != 0) {
                        nb_box = addbox(
                          mesh(Gi[0], Gi[1] + inc[1], Gi[2]), Tabox, nb_box);
                        axe[0]--;
                        axe[1]--;
                        diag--;
                }
                if (inc[2] != 0) {
                        nb_box = addbox(
                          mesh(Gi[0], Gi[1], Gi[2] + inc[2]), Tabox, nb_box);
                        axe[1]--;
                        axe[2]--;
                        diag--;
                }
                ...
                for (b = 0; b < nb_box; b++) { // loop sur les boites dans denv
                        ...
                        for (box->Ldiff.debut(); !box->Ldiff.finito();
                             box->Ldiff.suivant()) {
                                ...
                                /*
                                  #ifdef _AntiDoublon
                                  //ramene le tps a celui de toute la scene pour
                                  le tps de gestion de boucle
                                  // a gerer par la matrice en testant si la
                                  M(diffR,diffE) est deja prise? tq pas doublon
                                  (a voir le temps de parcours de la matrice
                                  creuse
                                  //printf(" ** card(L2)=%d\n",L2.card());
                                  rechauffe=false;
                                  if(!L2.est_vide()) {
        ...
        printf("[calc_FF_Bfar()] \n  mean(diff/patch) = \t%lf\n  "
               ...
               nb_test / (double)nb_rec,
               nb_emi / (double)nb_rec,
               cum_box / (double)nb_rec);
} // Canopy::calc_FF_Bfar()
