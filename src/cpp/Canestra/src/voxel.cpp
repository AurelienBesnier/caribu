...
template<class Type>
inline char
T_niveau_egal(Type* val, Type* val_max)
{
        return (((val[0] == val_max[0]) && (val[1] == val_max[1]) &&
                 (val[2] == val_max[2]))
                  ? 1
                  : 0);
}
...
Voxel::~Voxel()
{
        ...
        if (1) { // Debug MCHA 151203
                ...
                // MC161203: ne marche pas, car le delete ne doit pas mettre a
                // null MC161203 : gestion a la mano des desallocation par un
                // tableau de pointeur NB une facon plus propre serait de gerer
                // dans BSP un nbre d'instance
...
void
Voxel::division(char axe, unsigned char* niveau_sub_max, BSP* B)
{
        BSP* B1;
        BSP* B2;
        int i;
        ...
        niveau_sub_max[axe]++;
        if (verbose > 3)
                Ferr << "[division] DEBUT axe " << (int)axe
                     << " : niveau subdiv = " << (int)niveau_sub_max[axe]
                     << "\n ";
        fflush(stderr);
        for (i = 0; i < 3; i++) {
                taille_min[i] =
                  fabs((B->maxi(i) - B->mini(i)) - taille_vox) < 1e-4;
                if (verbose > 3)
                        Ferr << "--> delta[" << i
                             << "] = " << B->maxi(i) - B->mini(i) << "\n";
        }
        ...
        if (taille_min[0] && taille_min[1] &&
            taille_min[2]) { // Arret si BSPfeuille==voxel
                if (verbose > 3)
                        Ferr << "*** Voxel[division] Rgt boite : Niv_Max - "
                                "nb_diff = "
                             << B->nb_diffuseur() << "\n";
                numerotation(B);
        } else {
                if (B->nb_diffuseur() <= SEUIL_NB_DIFF) {
                        ...
                        // Ferr <<"decoupage de la boite selon l'axe "  <<
                        // (int)axe<<" " ;

                        // colle la limite des 2 boites-fille a la grille de
                        // voxels
                        frontier = (B->maxi(axe) + B->mini(axe)) / 2.0;
                        indi = coord(axe, frontier);
                        if (verbose > 3)
                                Ferr << " [division] coupure selon l'axe "
                                     << axe << " au debut de la boite " << indi
                                     << "\n";
                ...
                ListeD<Diffuseur*>& Ldiff)
{
        ...
        /* Lbox.debut();
           B=Lbox.contenu();
           Ferr <<"Voxel[creation]boite.nb_diffuseur= "  <<
           B->nb_diffuseur()<<"\n" ;
        */
        if (verbose) {
...
Voxel::numerotation(BSP* B)
{
        ...
        for (ii = 0; ii < 3; ii++) {
                ...
                if ((B->mini(ii) - (lim[ii][0] * taille_vox + O[ii])) >
                    taille_vox / 2.0)
                        lim[ii][0]++;
                lim[ii][1] = (int)((B->maxi(ii) - O[ii]) / taille_vox);
                if ((B->maxi(ii) - (lim[ii][1] * taille_vox + O[ii])) >
                    taille_vox / 2.0)
                        lim[ii][1]++;
                /*
                  ...
                  << ii<<"]="  <<  lim[ii][1]<<"\n" ;
                  Ferr <<" lim_min["  << ii<<"]="  <<
                  lim[ii][0]*taille_vox+O[ii]
                  <<" - lim_max["  << ii<<"]="  <<
                  lim[ii][1]*taille_vox+O[ii]<<"\n" ;
                */
        ...
        for (i = 0; i < SI.maxi()[0]; i++)
                for (j = 0; j < SI.maxi()[1]; j++)
                        for (k = 0; k < SI.maxi()[2]; k++) {
                                ...
                                B = SI(i, j, k);
                                // Ferr <<" Boite "  <<  B->nom()<<"
                                // contenanFerr << __FILE__ " : "<< __LINE__ <<
                                // '\n' ;t "  << B->nb_diffuseur()<<"
                                // diffuseurs\n" ;
                        }
        Ferr << "Voxel[visu()] FIN"
...
//-****************** Voxel::construction()  *****************
void
Voxel::construction(reel* bornemin,
                    reel* bornemax,
                    double Renv,
                    ListeD<Diffuseur*>& Ldiff)
{
        ...
        // calcul des dimensions de la grille en fct de Renv et ajustement des
        // bornes
        grille_par = fopen(grille_nom, "r");
        if (grille_par == NULL) {
                Ferr << "<!> Voxel::construction() -> fichier \"" << grille_nom
                     << "\" pas trouve \n   => valeurs par defaut des "
                        "parametres de subivision adaptative\n";
                SEUIL_NB_DIFF = 50; // arret de la subdivision
                ratio = 1; // ration entre arete de la grille et diametre de la
                           // sphere englobante
                // strcpy(grille_nom,"Valeurs par defaut");
                Ferr << "Default threshold "
        ...
        for (i = 0; i < 3; i++) {
                ...
                // CF 2016 : meme si reste = 0, il faut aussi ajouter un voxel
                // de plus
                ...
                bornemax[i] += delta[i];
                bornemin[i] -= delta[i];
                Ferr << "xxx  bmin[" << i << "]= " << bornemin[i] << " - bmax["
                     << i << "]= " << bornemax[i] << " - nb_vox[" << i
                     << "]= " << nb_vox[i] << " \n";
        }
        ...
        if (verbose)
                Ferr << "Voxel[construction] nbre de boites cre'e'es = "
                     << num_box << "\n";
        // visu();
