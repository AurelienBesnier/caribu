...
Tremove(char* buffer)
{
        Ferr << ">>> bzh.cpp: Tremove()- destruction par remove() du fichier "
             << buffer << '\n';

        if (remove(buffer)) {
                switch (errno) {
                        case EACCES:
                                Ferr << ">>> bzh.cpp: Tremove()- File "
                                        "protected against writing !"
                                     << '\n';
                                cout << ">>> bzh.cpp: Tremove()- File "
                                        "protected against writing !"
                                     << '\n';
                                break;
                        case ENOENT:
...
placenette(char* ficname)
{
        /*  char cmd[255];
            ...
            if(k==0){
            Ferr<< " <!> Danger: fichier "<<ficname<<" existant =>
           destruction!"<<'\n';
...
void
hdmat_init(char* dir, char* rootname)
{
        char tempo[128];
        ...
        if (true || verbose)
                Ferr << "HDMatrices : NZ=" << pcNzName << ", DG=" << pcDgName
                     << ", BF=" << pcBfName << '\n';
        // cout <<"HDMatrices : NZ="  << pcNzName<<", DG="  << pcDgName <<",
        // BF=" << pcBfName<<'\n' ;
...
void
hdmat_majname(char* dir, char* suff)
{
...
void
hd_calc_Bfar(VEC* Cenv, char* pcEnvName, Diffuseur** TabDiff, double Eclt)
{
        int i, is, j, Nc, iff, nbp,
