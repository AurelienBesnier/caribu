...
class Source_Lumiere
{
      public:
        ...
        Source_Lumiere(int alancer = 10) { nb_photons = alancer; }
};
...
class SoleilSS : public Source_Lumiere
{
      ...
      public:
        ...
        void init(int&,
                  Vecteur&,
                  double*,
                  double*); // meme utilisation que le 1er constructeur
        virtual Param_Inter lache_photon(); // donne un photon a lancer
      private:
        // fonctions internes
        Point perturbe(Point&);   // perturbation du point de depart
        void init_grille2d();     // calcul une repartition des points de depart
                                  // suivant une grille 2D
