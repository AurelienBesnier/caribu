...
VEC*
hd_mv_mlt(SPMAT* A, VEC* x, VEC* out)
{
        int tamp[2], i, is, j_idx, j, n, iff, nd,
          *diag; // ip : indice prim, is indice face
        Real sum, *x_ve, sum2;
        ...
        for (i = 0; i < nd; i++) {
                ...
                // printf(".. i=%d, is=%d, diag=%d, n=%d, nd=%d
                // ",i,is,diag[i+1],n,nd); fflush(stdout);
        ...
        int* steps)
{
        ...
        if (ip->k <= 0)
                error(E_BOUNDS, (const char*)"mgcr");
        ...
        for (ip->steps = 0; ip->steps < ip->limit;) {
                ...
                for (i = 0; i < ip->k && ip->steps < ip->limit; i++) {
                        ...
                        if (i < ip->k - 1) {
                                s.ve = N->me[i + 1]; /* pointer to a row of N
                                                        (=s_{i+1}) */
                                v_copy(rr, &s);      /* s_{i+1} = B*A*s_i */
                                for (j = 0; j <= i - 1; j++) {
                                        v.ve =
                                          N->me[j + 1]; /* pointer to a row of N
                                                           (=s_{j+1}) */
                                        /* beta->ve[j] = in_prod(&v,rr); */ /* beta_{j,i}
                                                                             */
                                        /* modified Gram-Schmidt algorithm */
                                        beta->ve[j] =
                                          in_prod(&v, &s); /* beta_{j,i} */
                                ...
                                sv_mlt(
                                  1.0 / nres, &s, &s); /* normalize s_{i+1} */

                                v.ve = N->me[0];
                                alpha->ve[i] = in_prod(
                                  &v, &s); /* alpha_i = (s_0 , s_{i+1}) */
...
void
print_hd_mat(Diffuseur** TabDiff)
{
        int i, is, j_idx, j, n, iff, nd,
          *diag = NULL; // i : indice prim, is indice face
        double dff;     // pour passer d'un FF en pixel a un FF reel
        double rho[2], tau[2], po;
