...
class Image
{
      ...
      public:
        Image() {}
        Image(int x, int y, char* nom = (char*)"out.ppm") { init(x, y, nom); }
        ...
        Image& operator=(Image& img)
        {
                ...
                cout.flush();
                Image* pim =
                  new Image(img.btm.maxi()[0], img.btm.maxi()[1], img.filename);
                /*        strcpy(filename,img.filename); cout<<"Image[=]
                   :"<<filename<<endl;
        ...
        double& val(int i, int j) { return btm(i, j); }
        ...
        void sauve(char* name = (char*)" ")
        {
                ...
                // cout<<"Image[sauve] ficname = "<<name<<" - bitmax =
                // "<<bitmax<<endl;
                fic << "P5\n";
        ...
        int taille(int i) { return btm.maxi()[i]; }
        void free() { btm.free(); }
}; // Image PPM
...
class RGB
{
      ...
      public:
        RGB() {}
        RGB(int x, int y, char* nom = (char*)"out.ppm") { init(x, y, nom); }
        // typage AH 02 2001
        void init(int x, int y, char* nom = (char*)"out.ppm")
        {
                btm.alloue(x, y, 3);
        ...
        RGB& operator=(RGB& img)
        {
                ...
                cout.flush();
                RGB* pim =
                  new RGB(img.btm.maxi()[0], img.btm.maxi()[1], img.filename);
                /*        strcpy(filename,img.filename); cout<<"RGB[=]
                   :"<<filename<<endl;
                        btm.alloue(img.btm.maxi()[0],img.btm.maxi()[1]);
                */
        ...
        void fixmax(double maxxi) { bitmax = maxxi; }
        double& val(int i, int j, int k) { return btm(i, j, k); }
        ...
        { // raus((val<0&&val>255)," RGB[maj] val  non valide\n");
                btm(i, j, 0) = tval[0];
                bitmax = (bitmax < tval[0]) ? tval[0] : bitmax;
                btm(i, j, 1) = tval[1];
                bitmax = (bitmax < tval[1]) ? tval[1] : bitmax;
                btm(i, j, 2) = tval[2];
                bitmax = (bitmax < tval[2]) ? tval[2] : bitmax;
        }
                ...
                for (j = 0; j < btm.maxi()[1]; j++)
                        for (i = 0; i < btm.maxi()[0]; i++)
                                for (k = 0; k < 3; k++) {
                                        /*
                                          bit= (unsigned char)
                                          (btm(i,j,k)/bitmax*255);
                                          bit=(bit>=255)?255:bit;
                                          // printf("btm(%d,%d,%d)=%d -
                                          %f\n",i,j,k,bit,btm(i,j,k));
                                          // fic.write(&bit,1);  //sizeof(int));
                                          const char *ccTmp = &bit ; // HA 11
                                          2003 fic.write(ccTmp,1);
                                          /*/
                                        pix = (unsigned int)(btm(i, j, k) /
                                                             bitmax * 255);
                                        pix = (pix >= 255) ? 255 : pix;
                                        snprintf(
                                          ccTmp, sizeof(ccTmp), "%ud", pix);
                                        fic.write(ccTmp, 1);
                                        /**/
                                }
                fic.close();
