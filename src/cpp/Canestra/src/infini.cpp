/*                  Infini.C - MC96
   ...
   MC09: Bug fixed: bias on infinitise() for high zenithal angles and particular
   azimuthal angles.
 */
...
// proto
void
lateral(int x, int y, char idx);
void
pave(int x, int y, char idx);

// local function
bool
out(int i, int j, int moving_i, int moving_j)
{
        int k, l;
        ...
        // cout << "\ndehors[1]: " << dehors[1];
        // cout << "\ni,j ("<<i<<","<<j<<") out ["<<Ti<<","<<Tj<<"] ? ==> " <<
        // ((dehors[0] && dehors[1]) ? "true" : "false") << "\n";
        //  BUG BUG BUG BUG  - MC09
        // MC96   return dehors[0]||dehors[1]; // BUG BUG theta fort > 70� !!! -
        // MC09
        //  MC09 return (dehors[0] && dehors[1]); // Bug qd phi=0�, boucle
        //  infinie...
...
lateral(int x, int y, char idx)
{
        if (out(x, y, tr[idx], tr[idx + 1]))
                return;
        if (verbose > 1)
                printf("lateral (%d,%d,%d)-", x, y, idx);
        zbuf(x, y);
           ...
           bool dupli)
{
        ...
        */
        ...
        // MC09: debug / fort angles zenitaux => ne chga erien mais plus
        // rigoureux
