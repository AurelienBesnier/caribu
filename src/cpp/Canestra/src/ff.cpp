...
static double* mpei;
static double &mpe_00 = mpe[0][0], &mpe_01 = mpe[0][1], &mpe_02 = mpe[0][2],
              &mpe_03 = mpe[0][3];
static double &mpe_10 = mpe[1][0], &mpe_11 = mpe[1][1], &mpe_12 = mpe[1][2],
              &mpe_13 = mpe[1][3];
static double &mpe_20 = mpe[2][0], &mpe_21 = mpe[2][1], &mpe_22 = mpe[2][2],
              &mpe_23 = mpe[2][3];
...
static inline double
env(int i, int j, double& denv, signed char tr)
{
        if (Nc == 0)
                return 0.0;
        else {
                ...
                if (M[2] <= Tenv(0, 2) || M[2] > Tenv(Nc, 2))
                        ...
                        if (layer == 0.0) {
                                ...
                                transm = ((layer * Tenv(il, up)) +
                                          (1 - layer) * Tenv(il + 1, up));
#ifdef _HD
                        ...
                        //  MCsail produit des flux et non plus des valeurs
                        //  relatives
...
static inline void
centre(Point& M, Point& A, Point& B, Point& C)
{
...
#else
void
init_NFF(char* EnvName, double* Esource, double& Rsph, bool bias)
{
#endif
        ...
        if (verbose)
                Ferr << "*** Resolution du disque de projection :  " << NB
                     << "x" << NB << '\n';
        MB = NB;
        DbuffS.alloue(NB, MB);
        DbuffI.alloue(NB, MB);
        ...
        */
        ...
        if (EnvName == NULL)
                ...
                FILE* ffb;
                if (verbose > 5)
                        Ferr << "FF.cpp: init_NFF(): file " << pcBfName
                             << " open for writing\n";
...
init_proj(Diffuseur* diffR)
{
        ...
        u.formation_vecteur(diffR->primi().sommets(1),
                            diffR->primi().sommets(0));
        u.normalise();
        w = diffR->normal();
...
void
proj_ortho(Diffuseur* E, reel* inc)
{
        int i, j, i1, i2, jm, j0;
        signed char pasglop = 0, in = 0, ins = 0, nbd, face,
                    iz0[2] = { -1, -1 }, izm[2] = { -1, -1 },
                    izp[2] = { -1, -1 }, t;
        ...
        if (E->isopaque()) {
                ...
                if (er.prod_scalaire(E->normal()) > 0) {
                        if (verbose > 1)
                                printf("*** pas vu : opak par derriere\n");
                        return;
                }
        }
        Point B;
        ...
        switch (pasglop) {
                ...
                default: // cas ou pas besoin de subdiviser
                        ...
                        nbd = 1;
                        izm[0] = (pasglop == 3 || pasglop == 6 || pasglop == 9)
                                   ? 0
                                   : 1;
                        mid[0] = Pp[0];
                        mid[1] = Pp[1];
        ...
        for (t = 0; t < nbd; t++) { // boucle de subdivion du triangle
                ...
                for (i = 0; i < 3; i++) {
                        ...
                        // printf("\n");
                        b[i] = -1; // b contiendra les coeff de l'equation du
                                   // plan du triangle E
                        // printf("PROJ_ORHTO() : Pp[%d] = %f, %f,
                        // %f\n",i,Pp[i][0],Pp[i][1],Pp[i][2]);
                } // for i

                // printf("PROJ_ORHTO() : diffE no. %d : sens =
                // %d\n",E->num(),izm[0]);
                if (((izm[0] == 0) || (transp && (izm[0] == 1))) &&
                    solve3(M, b)) {
                        ...
                        // printf("PROJ_ORHTO() : P = %f, %f,
                        // %f\n",P[0],P[1],P[2]);
                        loc2sph(P, P);
                        // Warning this is a bug !
                        // :extrem=sqrtab(sph2img(P[0]),sph2img(P[1]));
                        ...
                        for (i = 0; i < 3; i++) { // loop arete
                                ...
                                if (init_MPE(i, j)) {
                                        Vecteur er;
                                        er.formation_vecteur(O, E->centre());
                                        Ferr << " Pas possible de calculer "
                                                "init_MPE : triangle no. "
                                             << E->num()
                                             << "  biz? en le projetant sur "
                                             << receiver->num()
                                             << ", qui sont a une distance de "
                                             << er.norme() << '\n';
                                        // exit(0);
                                        return;
                                }
                                loc2sph(Pp[i], Ps[i]);
                                // printf("coord sph : Ps[%d] = %f, %f,
                                // %f\n",i,Ps[i][0],Ps[i][1],Ps[i][2]);//valeur
                                // de MPE au centre du TRiangle
                                mpei[3] = mpei[0] * P[0] + mpei[1] * P[1] +
                                          mpei[2] * extrem;
                                // printf("MPE[%d][3] = %lf\n",i, mpei[3]);
                        } // lopp arete
                        ...
                        for (i = 0; i < 3; i++) { // loop arete
                                ...
                                div_extr =
                                  tab_mpe2[0] + tab_mpe2[1] + tab_mpe2[2];
                                // cout<<"a"<<i;
                                for (j = 0; j < 2; j++) {
                                        // cout<<" - axe "<<j;
                                        ...
                                        if (den_extr == 0.) {
                                                Vecteur er;
                                                er.formation_vecteur(
                                                  O, E->centre());
                                                Ferr << "=> ARRET proj_ortho() "
                                                        "--->arete no. "
                                                     << i
                                                     << " -  den_extr=0!\n";
                                                Ferr << "   triangle no. "
                                        ...
                                        if ((pos_extr > Ps[i1][j]) &&
                                            (pos_extr < Ps[i2][j])) {
                                                // cas ou le max est dans
                                                // l'interval
                                                REt[jm] = (extrem > maxP)
                                                            ? extrem
                                                            : maxP;
                                                REt[jm - 1] = (extrem < minP)
                                                                ? extrem
                                                                : minP;
                                        } else {
                                                REt[jm] = maxP;
                                        ...
                                        // ordo(min)="<<Ps[i1][kk]<<",
                                        // ordo(max)="<<Ps[i2][kk]<<",
                                        // ordo(tgte)="<< extrem<<", max="<<
                                        // REt[jm]<<", min="<<REt[jm-1]<<endl;
                                }
                        } // loop arete 2
                        ...
                        // \n",RE[0],RE[1],RE[2],RE[3]) ; printf("REimg : %d ;
                        // %d - %d ; %d \n",i1,i2,j0,jm);
                        for (i = i1; i <= i2; i++) {
                                x = img2sph(i);
                                for (j = j0; j <= jm;
                                     j++) { // loop sur les proxels du RE
                                        if (sqrtab(i, j) !=
                                            -1) { // dans le cercle
                                                y = img2sph(j);
                                                // debug pour voir le carre
                                                // englobant :
                                                // DbuffS(i,j)=receiver;
                                                //  MC debug 151203 in=3;
                                                int in = 3;
                                                extrem = sqrtab(
                                                  i, j); // extrem var. tempo
                                                         // pour economiser les
                                                         // appel a sqrtab()
                                                // Ferr << __FILE__ " : "<<
                                                // __LINE__ << '\n' ;
...
#ifdef _HD
void
NFF(int i_sup, int i_inf, VEC** Cfar)
{
        int ii, nnz = 0;
#else
void
NFF(SPMAT* FF, int& i_sup, int& i_inf, VEC** Cfar)
{
#endif
        ...
        for (i = 0; i < NB; i++) {
                for (j = 0; j < NB; j++) {
                        if (sqrtab(i, j) >=
                            0) { // cas ou je suis dans le disque de projection
                                E = DbuffS(i, j);
                                // printf("==> NFF(%d,%d) = %d :
                                // ",i,j,(E==NULL)?0:1);
                                if (E != NULL) {
                                        ...
                                        FFenv =
                                          env(i, j, denv, 1) *
                                          dFF; // xier par M_PI si luminance
                                        // printf("FFenv(sup)=%lf\n",FFenv);
                                        Cfar[0]->ve[i_sup] +=
                                          FFenv * rho[0]; // face sup en refl
                                        far_cum++;
                                        if (FFenv > 0)
                                                vfar_cum++;
                                        // printf("\t (%d,%d) en far\n",i,j);
                                ...
                                if (transp) {
                                        ...
                                        if (E != NULL) {
                                                ...
                                                FFenv = env(i, j, denv, -1) *
                                                        dFF; // pense a xier par
                                                             // rho ou tau
                                                Cfar[0]->ve[i_inf] +=
                                                  FFenv *
                                                  rho[1]; // face inf en refl
                                                Cfar[0]->ve[i_sup] +=
                                                  FFenv *
                                                  tau[1]; // face sup en trans
                                                far_cum += 2;
                                                if (FFenv > 0)
                                                        vfar_cum += 2;
                                        }
...
#ifdef _HD
        FILE* ffb;
        //  if(verbose>5)  Ferr<<"FF.cpp: NFF(): file "<<pcNzName<<" open for
        //  writing\n";
        ...
        for (i = 0; i < nb_face; i++)
                if (ligne(i) != 0) {
                        ...
                        tamp[0] = i;
                        tamp[1] = ligne(i);
                        // if(i_sup/2==29) printf(" rec %d  - emit = %d - iff =
                        // %d\n",i_sup/2,i,ligne(i));
...
#define NONZERO(A)                                                             \
        ...
        FILE* diagb;
        Ferr << "FF.cpp: stat_NFF(): file " << pcDgName
             << " open for writing\n";
