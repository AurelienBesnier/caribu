...
Soleil ::Soleil(int a, Vecteur b, double* bmin, double* bmax)
  : Source_Lumiere(a)
{ //  cout<<"Soleil [cstructor] bmax[2] = "<<bmax[2]<<" - bmaxZ =
  //  "<<bornemax[2]<<endl;
        init(a, b, bmin, bmax);
}

void
Soleil ::init(int& n, Vecteur& dir, double* bmin, double* bmax)
{
        Vecteur V;
        dir.normalise();
...
{
        Point P;
        Vecteur V(a.tirage() * (bornemax[0] - bornemin[0]),
                  a.tirage() * (bornemax[1] - bornemin[1]),
                  0.0);
        reel poids = 1.0;
        Param_Inter photon;
        P = origine_soleil + V;
...
void
SoleilSS ::init(int& n, Vecteur& dir, double* bmin, double* bmax)
{
        Vecteur V;
        dir.normalise();
        // cas d'un direction oblique
...
SoleilSS ::init_grille2d()
{
        ...
        xi = -1;
        cout << "SoleilSS[init_2d] pt[2] =" << pt[2]
             << " - bmaxZ = " << bornemax[2] << endl;
}

void
SoleilSS ::parcours_grille2d()
{
        pt[0] += Delta[0];
        xi++;
        if (xi >= n[0]) {
