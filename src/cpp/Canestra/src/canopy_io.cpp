...
Actop*
lectop(ifstream& fopti, bool opac = false)
{
        ...
        switch (c) {
                ...
                case 's': // specular (Fresnel's law + attenation(van der Bild)
                        syntax_error(
                          (char*)" .opt : Speculaire pas implemente\n");
                        break;
                case 'm': // mixte (Lambert + specular)
                        if (verbose)
                                Ferr << "*** Attention : prise en compte "
                                        "uniquement de la"
                                     << " partie diffuse"
                                     << "\n";
                ...
                case 'n': // neural network's regression
                        syntax_error(
                          (char*)" .opt :  description des prop. opt. par "
                                 "reseau de neurones pas implementee\n");
                        break;
                default:
...
not_yet(char* type)
{
        Ferr << "desole mais le type\"" << type
             << "\" n'est pas encore implemente : Ligne ignoree... \n";
} // not_yet()

inline char*
endline(ifstream& fin)
{
                  ...
                  Diffuseur**& TabDiff)
{
        ...
        do {
                ...
                switch (c) {
                        ...
                        case 's':
                                if (ii == 0)
                                        syntax_error(nopti);
                                // Ferr <<"Canopy[parse_can]ficoptik : sol
                                // lu\n";
                                tabopaque(nbopt) = lectop(fopti, opak);
                                raus(tabopaque(nbopt) == NULL,
                                     "Canopy[parse_can] allocation tabopaque "
                                     "impossible!");
                                nbopt++;
                                fopti.getline(line, 256);
                                break;
                        case 'e':
                                if (ii == 0)
                                        syntax_error(nopti);
                                // Ferr <<"Canopy[parse_can]ficoptik : espece no
                                // "<<nbopt<<'\n' ;//endl;
                                tabopaque(nbopt) = lectop(fopti, opak);
                                raus(tabopaque(nbopt) == NULL,
                                     "Canopy[parse_can] allocation tabopaque "
                                ...
                                raus(tabtransp(nbopt - 1, 0) == NULL,
                                     "Canopy[parse_can] allocation "
                                     "tabtransp_sup impossible!");
                                tabtransp(nbopt - 1, 1) =
                                  lectop(fopti); // face inf
                                raus(tabtransp(nbopt - 1, 1) == NULL,
                                     "Canopy[parse_can] allocation "
                                     "tabtransp_inf impossible!");
                                nbopt++;
                                fopti.getline(line, 256);
                                break;
                        default:
        ...
        do {
                ...
                switch (T) {
                        ...
                        case 'p':
                                ...
                                break;
                        case 'n':
                                not_yet((char*)"poly avec normales");
                                break;
                        case 'd':
                                not_yet((char*)"disque");
                                break;
                        case 'y':
                                not_yet((char*)"cylindre");
                                break;
                        case 's':
                                not_yet((char*)"sphere");
                                break;
                ...
                if (!valid)
                        ...
                        // format label : esp*1E11 + plante*1e6 + feuille*1E3 +
                        // triangle
                        //  MC Avril 98 : Pb sur sun de division de
                        //  tabid(0)/1e11 !!!!
                        //                => Creation de la var. double
                        //                espid=1e11
                        //  specie=(short)(tabid(0)/100000000000);
                        specie = (short)(tabid(0) / espid);

                        opak =
                          ((long)(tabid(0) / 1000) % 1000 == 0) ? true : false;
                        // MCjune08: nom=tabid(0)/1000;
                        nom = tabid(0);
                        /* debug ?
                              if (false){
                                if(opak)
                                  printf("===>  espece %d opaque
                           %c\n",specie,7); else printf("===>  espece %d
                           transparente %c\n",specie,7);
                              }
                        */
                        ...
                        if (min[0] > max[0]) { // primitive rejete
                                // Ferr <<" *****  Primitive rejete : libelle =
                                // "<<nom<<'\n' ;//endl;
                                rejet = true;
                        }
                        /* Traitement des a-cheval ici et non dans
                           BSP::volume_englobant, co parcinopy a cause de
                           visu3d.C*/
                        else if (infty) {
                                // printf("infty!!!!!");
                                Point G;
                        ...
                        if (rejet) {
                                ...
                                for (i = 0; i < 3; i++) {
                                        if (infty)
                                                i = 2;
                                        bornemin[i] =
                                          T_min(bornemin[i], min[i]);
                                        bornemax[i] =
                                          T_max(bornemax[i], max[i]);
                                }
                                /* ajout d'un diffuseur a la liste */
                                if (opak)
                                        diff =
                                          new DiffO(prim, tabopaque(specie));
                                else
                                        diff =
                                          new DiffT(prim,
                                                    tabtransp(specie - 1, 0),
                                                    tabtransp(specie - 1, 1));
                                assert(diff != 0);
                                // cout<<"numero = "<<diff->num()<<'\n' ;//endl;
                                diff->acv = acv;
                                Ldiff.ajoute(diff);
                                Ldiff0.ajoute(
                                  -1); // bon triangle : code label <0 - MC10
                                nbp++;
                        } // else rejected primi
                        tabid.free();
                } // else  !valid
        ...
        fgeom.close();
        //  if(rejet) cout <<"Canopy[parse_can] *************  Segment(s)
        //  rejete(s) *******\n";
        if (verbose)
                cout << "Canopy [parse_can] nbre de primitives  ss sol = "
                     << nbp << '\n'; // endl;
        if (verbose > 1)
                cout << "Canopy [parse_can] surface max primitive      = "
                     << smax << '\n'; // endl;
        ...
        if (sol != 0) {
                ...
                nbs = (int)(sqrt((bornemax[0] - bornemin[0]) *
                                 (bornemax[1] - bornemin[1]) / 4 / smax)) +
                      1;
                printf("* surface-based number of soil triangles = %d\n",
                       nbs * nbs * 2);
                if (sol > 0 && sol < nbs * nbs * 2) {
                        nbs = (int)sqrt(sol / 2.);
                        printf("<!>The mesh of soil use the real threshold "
                               "value (%d T <= %d)\n",
                               sol,
                               nbs * nbs * 2);
                }
                ...
                for (i = 0; i < nbs; i++) {
                        for (j = 0; j < nbs; j++) {
                                ...
                                { // triangle du haut
                                        ...
                                        ligne << (p[1] + dy) << " ";
                                        ligne << bornemin[2] << "  ";
                                        // gerer des #define si istringstream
                                        // passe pas avec Bcc32 memes remarques
                                        // que dans le bloc precedent
                                        string sTmp = ligne.str();
                                        // iKompteur = sTmp.size();
                                        ...
                                        // isTmp >> pch ;
                                        //   ajout du sol 1 a la liste
                                        //  MC09 - BUG !!!! diff=new DiffO(new
                                        //  Polygone(pch,0,min,max),tabopaque(0));
                                        //	  diff=new DiffO(new
                                        //Polygone(sTmp,0,min,max),tabopaque(0));
                                        ////MC09 debug MC09
                                        //  string  -> char * 	  const char
                                        //  *c_str() const; ou const char
                                        //  *data() const;
                                        pch = (char*)sTmp.c_str();
                                        diff = new DiffO(
                                          new Polygone(pch, 0, min, max),
                                          tabopaque(0));
                                        // 90CM
                                        assert(diff != 0);
                                        Ldiff.ajoute(diff);
                ...
                if (verbose > 1)
                        cout << "Canopy [parse_can] nbre de primitives  avec "
                                "sol = "
                             << nbp << '\n'; // endl;
        }                                    // if sol
        for (i = 0; i < 3; i++) {
                ...
                if (verbose > 1)
                        printf("parse_can() : vmin[%d] = %g - vmax[%d] = %g \n",
                               i,
                               vmin[i],
                               i,
                               vmax[i]);
        ...
        if (nsolem != NULL) {
                ...
                fgeom >> T; // Ferr <<T;
                if (T != '#')
                        err_syntax((char*)"in the file .can, the first char IS "
                                          "NOT  '#'\n==> program aborted\n");
                fgeom >> nbcell;
                Ferr << " " << nbcell
                     << " cellules Solem positionees dans le couvert\n";
                if (nbcell == 0)
                        err_syntax((char*)"in the file .can, the second char "
                                          "should be the number of cell > "
                                          "0\n==> program aborted\n");
                delete endline(fgeom);
                ...
                for (i = 0; i < nbcell; i++) {
                        ...
                        switch (T) {
                                ...
                                default:
                                        err_syntax(
                                          (char*)"Erreur de syntaxe dans le "
                                                 "fichier can");
                        } // switch T
                        if (!valid) {
                                delete endline(fgeom);
                ...
                if (verbose > 1)
                        cout
                          << "Canopy [parse_can] nbre de capteurs virtuels = "
                          << nbcell << '\n'; // endl;
        }                                    // if capteur virtuel
        else {
                nbcell = 0;
        ...
        radim = diff->idx;
        if (verbose > 1)
                cout << "Canopy [parse_can] nbre de faces visibles = " << radim
                     << '\n'; // endl;
        if (verbose)
                cout << "Canopy [parse_can] nbre de primitives = "
                     << Ldiff.card() << '\n'; // endl;
        ...
        for (Ldiff.debut(), i = 0; !Ldiff.finito(); Ldiff.suivant()) {
                diff = Ldiff.contenu();
                // Ecriture du .can (apres nettoyage de parse_can et ajout du
                // sol)
                prim = &(diff->primi());
                fprintf(fcan,
                        "p  1 %.0f %d \t",
                 ...
                 Diffuseur**& TabDiff)
{
        ...
        assert((hSharedSegIn = OpenFileMapping(
                  FILE_MAP_ALL_ACCESS, FALSE, clef_seg_in)) != NULL);

        lpSharedSegIn = MapViewOfFile(
          hSharedSegIn, FILE_MAP_ALL_ACCESS, 0, 0, SEGSIZE * sizeof(Patch));
        ...
        do {
                ...
                switch (c) {
                        ...
                        case 's':
                                ...
                                tabopaque(nbopt) = lectop(fopti, opak);
                                raus(tabopaque(nbopt) == NULL,
                                     "Canopy[read_shm] allocation tabopaque "
                                     "impossible!");
                                nbopt++;
                                fopti.getline(line, 256);
                                break;
                        case 'e':
                                if (ii == 0)
                                        syntax_error(nopti);
                                // Ferr <<"Canopy[read_shm]ficoptik : espece no
                                // "<<nbopt<<'\n' ;//endl;
                                tabopaque(nbopt) = lectop(fopti, opak);
                                raus(tabopaque(nbopt) == NULL,
                                     "Canopy[read_shm] allocation tabopaque "
        ...
        if (name8 != NULL) {
                ...
                if (true || verbose)
                        printf(
                          "INFTY read_shm() : %s =[%.3f,%.3f]x[%.3f,%.3f]\n",
                          name8,
                          bornemin[0],
                          bornemax[0],
                          bornemin[1],
        ...
        for (it = 0; it < Nt; it++) {
                ...
                specie =
                  (short)(fabs(double(Ts[it].t))); // HA cast double 11 2003
                ...
                rejet = false;
                if (min[0] > max[0]) { // primitive rejete
                        Ferr << " *****  Primitive rejete car min>max!?: "
                                "libelle = "
                             << nom << " - No=" << it << '\n';
                        rejet = true;
                }
                /* Traitement des a-cheval ici et non dans
                   BSP::volume_englobant, co parcinopy a cause de visu3d.C*/
                else {
                        if (infty) {
                                Point G;
                                ...
                                if (G[0] > bornemin[0] && G[1] > bornemin[1] &&
                                    ...
                                    G[1] <= bornemax[1]) {
                                        ...
                                        rejet = true;
                                        Ferr << "* Prim no " << it
                                             << ": Centroide G hors cube\n";
                                        Ferr << " bmin=" << bornemin[0] << ","
                                             << bornemin[1]
                                             << " - bmax=" << bornemax[0] << ","
                                             << bornemax[1]
                                             << "\n Centroid G =";
        ...
        if (rejet) {
                ...
                snprintf(
                  Tmsg,
                  sizeof(Tmsg),
                  ">>>  Canopy[read_shm] ****** %d  rejected triangles ****",
                  Nrejet);
                cout << Tmsg << "\n";
                Ferr << Tmsg << "\n";
        }
        if (verbose)
                cout << "Canopy [read_shm] nbre de primitives  ss sol = " << nbp
                     << '\n'; // endl;
        if (verbose > 1)
                cout << "Canopy [read_shm] surface max primitive      = "
                     << smax << '\n'; // endl;
        // sol
        printf("**** sol=%d\n", sol);
        if (sol) {
                ...
                if (true || verbose)
                        cout
                          << "Canopy [read_shm] nbre de primitives  avec sol = "
                          << nbp << '\n'; // endl;
        }                                 // if sol
        for (i = 0; i < 3; i++) {
                vmin[i] = bmin[i] = bornemin[i];
                vmax[i] = bmax[i] = bornemax[i];
                if (verbose > 1)
                        printf("read_shm() : vmin[%d] = %g - vmax[%d] = %g \n",
                               i,
                               vmin[i],
                               i,
                               vmax[i]);
        ...
        if (nsolem != NULL) {
                ...
                fgeom.close();
                if (verbose > 1)
                        cout << "Canopy [read_shm] nbre de capteurs virtuels = "
                             << nbcell << '\n'; // endl;
        }                                       // if capteur virtuel
        else {
                nbcell = 0;
        ...
        radim = diff->idx;
        if (verbose > 1)
                cout << "nbre de faces visibles = " << radim;
        if (verbose) {
...
//%%%%%%%%%%%%%%%%%%%       isobary()       %%%%%%%%%%%%%%%%%%
inline Point
isobary(Point& A, Point& B, Point& C)
{
        Point G;
...
void
Canopy::xabs(char* nx3d, double* bornemin, double* bornemax, bool normee)
{
        ...
        for (i = 0, Ldiff.debut(); !Ldiff.finito(); Ldiff.suivant()) {
                ...
                for (j = 0; j < 3; j++, i++) {
                        fout << i << "\t"
                             << M * (prim.sommets(j)[0] - bornemin[0]) - 1000.0;
                        fout << "  "
                             << M * (prim.sommets(j)[1] - bornemin[1]) - 1000.0;
                        fout << "  "
                             << M * (prim.sommets(j)[2] - bornemin[2]) - 1000.0
                             << '\n'; // endl;
                }
                if (normee) {
                        G = isobary(
                          prim.sommets(0), prim.sommets(1), prim.sommets(2));
                        fout << i++ << "\t"
                             << M * (G[0] - bornemin[0]) - 1000.0;
        ...
        fout << nb_prim << '\n'; // endl;
        for (i = 0, j = 0, Ldiff.debut(); !Ldiff.finito();
             Ldiff.suivant(), i++) {
                pdiff = Ldiff.contenu();
                color = (int)((double)(nbcol - 4) * (pdiff->nrj() - Emin) /
                              (Emax - Emin));
...
void
Canopy::xrad(char* nvar, double* bornemin, double* bornemax, bool normee)
{
        int i, j, nb_prim = 0, nb_seg = 0, nb_face = 0, color;
        double M, Emax = -1.0, Emin = 9.9e12;
        int nbcol = 203;
        ...
        fout << "#     " << nvar << '\n'; // endl;
        fout
          << "# genere par raddir - visu des radiosite d'un couvert - MC95\n";
        nb_prim = Ldiff.card();
        // Couleurs
        fout << "# number of colors used in object\n";
        fout << nbcol << '\n'; // endl;
        ...
        for (i = 0, j = 0, Ldiff.debut(); !Ldiff.finito();
             Ldiff.suivant(), i++) {
                pdiff = Ldiff.contenu();
                color = (int)((double)(nbcol - 4) * (pdiff->Radmax() - Emin) /
                              (Emax - Emin));
                fout << "# " << pdiff->primi().name() << '\n'; // endl;
                fout << i << "  " << color << " 3  " << j++ << "\t" << j++
                     << "\t" << j++ << '\n'; // endl;
