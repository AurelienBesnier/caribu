...
void
Camera::init(char* ficname, char* imgname)
{
        char buffer[80];
        ...
        cout << "verti " << verti[0] << " " << verti[1] << " " << verti[2]
             << endl;
        cout << " angle " << angle_vue << " - resolution " << res[0] << " "
             << res[1] << endl;
        cout << "focal " << foc << endl;
        fic.close();
        ...
        verti.normalise();
        cout << " up ortho " << verti[0] << " " << verti[1] << " " << verti[2]
             << endl;
...
void
Camera::capte(Rayon& strahl, Diffuseur* pinter, int nbray)
{
        Vecteur dir;
        Param_Inter oldpar;
        Diffuseur* pdif = NULL;
        ...
        oldpar = strahl.par();
        // oldpar.direct().show();
        // cout<<"Camera[capte] depart rayon : ori  ";
        // strahl.par().origin().show(); cout<<"Camera[capte] arrivee rayon:
        // oeil "; oeil.show();
        dir.formation_vecteur(strahl.par().origin(), oeil);
        dir.normalise();
        if (pinter->isopaque())
                if (dir.prod_scalaire(pinter->primi().normal()) <
                    0.0) { // cout <<"Camera[capte] Pas visble\n";
                           // pinter->primi().show();
        ...
        if (prim->intersect(strahl.par(), &I) <
            ER_MAEZ) { // cout<<"Camera[capte] dans la boite! taille
                       // "<<taille;I.show(); oldpar.direct().normalise();
                Point P(oldpar.direct());
                strahl.par().change_origine(P);
                dir.formation_vecteur(prim->operator[](0), I);
                //    dir.show();cout <<" x= "<<dir.prod_scalaire(u)<<" - y =
                //    "<<-dir.prod_scalaire(v)<<endl;
                double weight = pinter->lux(strahl.par());
                weight *=
                  taille * taille *
                  fabs(strahl.par().direct().prod_scalaire(prim->normal())) /
                  I.dist2(oldpar.origin());

                // maj de l'image
                int i, j;
                i = img->taille(0) - 1 -
                    (int)fabs(dir.prod_scalaire(u) / taille *
                              (img->taille(0) - 1));
                j = (int)fabs(dir.prod_scalaire(v) / taille *
                              (img->taille(1) - 1));
                img->inc(i, j, weight);
        }
...
void
Camera::capte_visi(Rayon& strahl, Diffuseur* inter, int nbray)
{
        Vecteur dir;
        Param_Inter oldpar;
        Point I;
        ...
        oldpar = strahl.par();
        // cout<<"Camera[capte_visi] depart rayon : ori  ";
        // strahl.par().origin().show();
        //    cout<<"Camera[capte_visi] arrivee rayon: oeil "; oeil.show();
        dir.formation_vecteur(strahl.par().origin(), oeil);
        dir.normalise();
        if (inter->isopaque())
                if (dir.prod_scalaire(inter->primi().normal()) <
                    0.0) {      // cout <<"Camera[capte_visi] Pas visble\n";
                        return; // pas visible
                }
        ...
        strahl.init_anamatides();
        //  cout<<"Camera[capte_visi] rayon out : ";
        //  strahl.par().direct().show();
        //    I=strahl.sortir();
        if (prim->intersect(strahl.par(), &I) <
            ER_MAEZ) { // cout<<"Camera[capte_visi] dans la boite! taille
                       // "<<taille;I.show();
                Point P(oldpar.direct());
                strahl.par().change_origine(P);
                dir.formation_vecteur(prim->operator[](0), I);
...
void
Camera::colorie_triangle(Diffuseur* pdif,
                         Point& a,
                         Point& b,
                         Point& c,
                         Tabdyn<double, 2>& Zbuf,
                         Tabdyn<Diffuseur*, 2>& Zprim,
                         Image* pimg)
// Vorsicht : Zbuf, Zprim, pdiff passes en variable globale (-lourd)
{
        ...
        cout << "\t C : ";
        c.show();
        cout << "\t deby = " << deby << " - finy = " << finy
             << " - yrel = " << yrel << endl;
        cout << "\t xL   = " << xL << " - xR   = " << xR << " -zL=" << zL
             << endl;
        cout << "\tpenteL= " << penteL << " -penteR= " << penteR
             << " - sens = " << sens << " - vab= " << vab << endl;

        //	boucle sur les lignes
        for (j = deby; j <= finy; j++) {
                iL = T_min(img->taille(0) - 1,
                           T_max(0, int(xL * K[0] + 0.5))); // centre de gravite
                iR = T_min(img->taille(0) - 1,
                           T_max(0, int(xR * K[0] - 0.5))); // dans le triangle
                //         cout<<"LOOP\t xL= "<<xL    <<" - xR= "<<xR<<
                //         "(iL,iR)="<<iL<<", "<<iR<<" -j= "<<j<<" -zL="<<zL<<
                //         endl;
                if (iL <= iR) {
                        dz = yrel * vbc / (xR - xL);
                        z = zL;
                        for (i = iL; i <= iR;
                             i++) { // cout<<"Capteur[colorie-triangle] (i,j) =
                                    // "<<i<<", "<<j<<endl;
                                // cout <<"Camera [colorie_triangle](i,j)
                                // ("<<i<<","<<j<<") z = "<<z<<"- Z =
                                // "<<Zbuf(i,j)<<endl;
                                if (z < Zbuf(i, j)) {
                                        //    cout <<"Camera
...
void
Camera::calc_visi(Liste<Diffuseur*>& Ldiff)
{
        ...
        */
        ...
        for (Ldiff.debut(); !Ldiff.finito(); Ldiff.suivant()) {
                ...
                */
                ...
                if (!pastoutvu) { // Tri sommets tq Pp[i][1]<<Pp[j][1]<<Pp[k][1]
                                  // ie A[1] < B[1] < C[1]
                        ...
                        if ((i != k) && !((A[0] == B[0]) && (B[0] == C[0])) &&
                            !((A[1] == B[1]) &&
                              (B[1] == C[1]))) { // Pts A,B,Cpas  alignes selon
                                                 // les axes Xou Y
                                // tri Ok
                                up = down = false;
                                if (A[1] == B[1]) // up
...
void
Camera::calc_stat(unsigned int niter = 0)
{
        raus(niter == 0,
             "Capteur[calc_stat] Doit avoir le nb d'iteration en parametre!");
        ...
        for (int i = 0; i < img->taille(0); i++)
                for (int j = 0; j < img->taille(1); j++) {
                        ...
                        sigma->maj(i,
                                   j,
                                   (sigma->val(i, j) / (double)niter - x * x) /
                                     ((double)niter - 1.0));
                }
        img->maxval();
        sigma->maxval();
