...
FileOk(char* nfic)
{
        if (access(nfic, R_OK) != 0) {
                fprintf(
                  stderr, "<!> Arret: fichier %s pas accessible !\n", nfic);
                exit(-1);
        }
} // FileOk()
...
main(int argc, char** argv)
{
        ...
        double max = -1;
        char *imgname = NULL, *maqname = NULL, *envname = NULL,
             *lightname = "std.light", *name8 = NULL, dataname[80],
             *optname = NULL;
        ...
        while ((c = option()) != EOF)
                switch (c) {
                        ...
                        case '8':
                                ...
                                break; // infini
                        case 'M':
                                max = atof(option.optarg);
                                break; //
                        case 'o':
                                imgname = option.optarg;
                                break;
                        case 'm':
                        ...
                        case 'a':
                                abs = true;
                                break; // cas de flux par feuilles (2 faces)
                        case 'h':
                                Ferr << "usage : " << argv[0]
                                     << " [hsa8<name>o<name>m<name>l<name>]\n";
                                return;
                        default:
                                Ferr << "usage : " << argv[0]
                                     << " [hsa8<name>o<name>m<name>c<name>l<"
                                        "name>]\n";
                                return;
                } // switch
        if (imgname == NULL) {
                imgname = new char[10];
                strcpy(imgname, "out.ppm");
        }
        if (maqname == NULL) {
                maqname = new char[10];
                strcpy(maqname, "test.can");
        }
        if (optname == NULL) {
                optname = new char[10];
                strcpy(optname, "fir.opt");
        }
