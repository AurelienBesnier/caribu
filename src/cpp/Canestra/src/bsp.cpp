...
void
BSP::destruction_boite()
{
        delete this;
}
// modif pour traiter un volume fixe par l'user  (cas infini) - MC 95
void
BSP::volume_englobant_scene(reel* bornemin,
                            reel* bornemax,
                            ListeD<Diffuseur*>& Ldiff_scene)
{
                ...
                for (Ldiff_scene.debut(); !Ldiff_scene.finito();
                     Ldiff_scene.suivant()) {
                        ...
                        /* test BPS deporte dans parse_can()
                           ...
                           if(fabs(bps[0])<nbs && fabs(bps[1])<nbs &&
                           fabs(bps[2])<nbs) {//Ote les exterieurs if( bps[0]<=0
                           && bps[1]<=0 ) {//ote les a cheval sur bornes max
                           */
                        Ldiff.ajoute(pdiff);
                        nb_diffuseurs++;
                        prim = &(pdiff->primi());

                        /*       fprintf(ftri," p  1 %ld %d
                         * \t",prim->name(),prim->nb_sommet()); HA */
        ...
        if (verbose > 1)
                printf("BSP[Vol englob] : nb_diff_scene= %d\n", nb_diffuseurs);
} // volume_englobant_scene(Liste)

int
BSP::partage_diffuseur(Diffuseur* no_diff, BSP* B1, int axe)
{
        if (no_diff->primi().tout_point_inf(B1->position_max[axe], axe))
                return 1;
        else if (no_diff->primi().tout_point_sup(B1->position_max[axe], axe))
                return 2;
        else
                return 0;
}
...
void
BSP::remplir_diffuseur(BSP* B1, BSP* B2, int axe)
{
        int partage;
        int i = 0;
        ...
        for (Ldiff.debut(); !Ldiff.finito(); Ldiff.suivant()) {
                /*  printf("BSP::remplir_diffuseur: diff %.0lf(%d) - axe=%d\n",
                    Ldiff.contenu()->primi().name(), Ldiff.contenu()->num(),
                   (int) axe); */
...
void
BSP::decoupe_boite(int axe, BSP* B1, BSP* B2, double frontier)
{
...
bool
BSP::contient(Point& P)
{
        int ii;
        bool dedans = true;
        for (ii = 0; ii < 3; ii++) {
                dedans &=
                  (P[ii] < position_max[ii]) && (P[ii] > position_min[ii]);
        }
        return dedans;
} // BSP::contient()
