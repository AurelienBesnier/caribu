 ...
 * MC spet 09: cretaion du fichier Etri.vec0 qui compatible avec le .can
 *d'entree
 *************************************************************/
...
#include <cassert> // associe a detection d'erreurs IO Windows
#endif
...
static double denv;
static bool ffseul, infty, geom, ordre1, ff_print, bio, byseg, byfile, radonly,
  memsize, bias;
static double seuil;
...
canestra(int argc, char** argv)
{
        ...
        cout << "\n Nombre de faces (2*L+S) = " << scene.radim << endl;
        cout.flush();
        if (true || verbose) {
                for (j = 0; j < 3; j++)
                        //  cout<<" main() : Bornemin["<<j<<"] = "<<bornemin[j]
                        //  <<"  Bornemax["<<j<<"] = "<<bornemax[j]<<endl;
                        Ferr << " main() : Bornemin[" << j
                             << "] = " << bornemin[j] << "  Bornemax[" << j
                             << "] = " << bornemax[j] << '\n';
        }
        if (denv < 0) { // Full-matrix case
                ...
                // denv=sqrt(denv)/2.;
                denv = sqrt(denv); // CF 2016, denv is the sphere radius, that
                                   // should be equal to the scene box diagonal
        ...
        if (!(radonly || ordre1 || denv == 0)) {
                clock.Start();
                // ex-pb effet de bord avec projplan car modifie les bornes =>
                // vmin et vmax
        ...
        do {
                ...
                // recommenter
                Ferr << "param. projplan : dir = (" << dir_source[0] << ","
                     << dir_source[1] << "," << dir_source[2]
                     << ") - Esun = " << Esource << '\n';
                for (i = 0; i < scene.radim; i++) {
                        if (Bsource[i] != 0.0) {
                                TabDiff[i]->activ_num(i);
                                if (Bsource[i] > 0)
                                        rho = TabDiff[i]->rho() * Bsource[i] /
                                              TabDiff[i]->surface();
                                else {
                                        TabDiff[i]->togle_face();
                                        rho = -TabDiff[i]->tau() * Bsource[i] /
                                              TabDiff[i]->surface();
                                }
                                // Cumule les contrib des differents angles
                                // solides
                                B0[0]->ve[i] += Esource * rho;
                                // Ferr <<"i="  << i<<" : Bsource="  <<
                                // Bsource[i]<<", B0="
                                //      << B0[0]->ve[i]<<"\n" ;
                                TabDiff[i]->active(0); // reactive le diff sur
                                                       // la face sup (defaut)
                        }
                }
        } while (flight);
        ...
        if (!ordre1) { // Calcul des rediffusions
                ...
                if (denv > 0) {
                        ...
                        if (!radonly) { // calcul de la matrice des FF
                                Ferr << " Version longue : calcul des FF et "
                                        "des Coeff de Bfar"
                                     << '\n';
                                // fflush(stderr); // Ferr.flush vient d'etr
                                // appele

                                hdmat_init(dirname, matname);
                                scene.calc_FF_Bfar(
                                  Cenv, &Esource, envname, bias, denv, nbsim);
                        } else {
                                // lecture de la mat. : maj des NzName, DgName
                                // et BfName et calcul des Bfar
                                Ferr << " Version courte : lecture des FF et "
                                        "des Coeff de Bfar\n";
                                hdmat_majname(dirname, matname);
                                if (envname != NULL)
                                        hd_calc_Bfar(
                                          Cenv[0], envname, TabDiff, Esource);
                        }
#else
                        SPMAT* FF;
...
#endif
                        if (verbose > 2)
                                Ferr << "\n--> initialiastion des vecteurs et "
                                        "matrices faites (alloc.)"
                                     << '\n';
                        // PAUSE(" ");
                        if (ffseul) {
                                // Ferr << __FILE__<< " : "<< __LINE__ << '\n' ;
...
#endif
                        clock.Stop();
                        Ferr << ">>> Canestra[main] FF et Bfar calcules  en "
                             << clock << '\n';
                        // fflush(stderr);
                        ...
                        if (ff_print) {
                                // Ferr << __FILE__<< " : "<< __LINE__ << '\n' ;
                                char carlu;
                                Ferr << "* ATTENTION - mode HD - Print la "
                                        "matrice a l'ecran\n"
                        ...
                        // Solve Ax=b; B precondionneur, tol seuil, limit
                        // nb_iter_max
                        clock.Start();
                        ...
                        if (num_steps < nb_iter)
                                Ferr << " MGCR CONVERGE en " << num_steps
                                     << " iteration(s) \n";
                        else
                                Ferr << " MGCRN'A PAS CONVERGE' ! \n";
                        Ferr
                          << ">>> Canestra[main] Resolution du SL par MGCR en "
                          << clock << '\n';
                } // if denv<>0
...
genres()
{
        // Impression des resultats : vecteur des  radiosites, if(bio) Eabs.dat
        // et Einc.dat
        int nbf = scene.radim - scene.nbcell;

        if (false &&
            !ordre1) { // genere les fichiers .dat de debug B0 et Bf generes
                if (envname != NULL) {
                        fres = fopen("Bf.dat", "w");
                        for (i = 0; i < nbf; i++)
                                fprintf(fres, "%lf \n", Cenv[0]->ve[i]);
        ...
        if (byfile) {
                fres = fopen("B.dat", "w");
                Ferr << "==> Impression des resultats radim=" << scene.radim
                     << ", nbcell=" << scene.nbcell << "\n";
                for (j = 0; j < nbf; j++) {
                        fprintf(fres, "%.10lf \n ", B[0]->ve[j]);
                }
        ...
        if (bio) {
                reel *Ei, *Eabs, D, D0, D1, r0 = 0, r1 = 0, t0 = 0, t1 = 0, E0,
                                            E1, Esol, Ssol;
                int ia, shmid2 = 0;
                ...
                if (byfile) { // by file
                        ...
                        ft = fopen("Etri.vec", "w");
                        fprintf(ft,
                                "# canestrad: can=%s F8=%s opt=%s light=%s : "
                                "denv=%.2f direct=%d \n",
                                maqname,
                                name8,
                                optname,
                                ...
                                (int)ordre1);
                        fprintf(ft,
                                "# label1 Area Eabs(E/s/m2) Ei(sup) Ei(inf) "
                                "(Ex=surfacic density of energy <nrj/s/m2>)\n");
                        // Version repreannt la liste initiale de triangle du
                        // .can pr PyCaribu
                        ...
                        fprintf(ft0,
                                ...
                                (int)ordre1);
                        fprintf(ft0,
                                "# No Label1 Area Eabs(E/s/m2) Ei(sup) Ei(inf) "
                                "(Ex=surfacic density of energy <nrj/s/m2>)\n");

                } else { // by shared memory

                        DecodeClefIn(&Nt, &clef_shm, clef_shm); // In caribu
                        // TEST: Ne plante plus si ouvre 1 segment =!= de celui
                        // de caribu
                        // clef_shm+=1 ;
                        cout << "==> print_Eabs(): Nt=" << Nt
                             << ", clef_shm=" << clef_shm << "\n";
                        Ferr << "==> print_Eabs(): Nt=" << Nt
                             << ", clef_shm=" << clef_shm << "\n";
#ifndef WIN32
                        // Mode Unix
                        shmid2 = shmget((key_t)clef_shm,
                        ...
                        if (shmid2 == -1) { // en cas de pb
                                ...
                                Ferr << "<!> Ouverture du segment partage no. "
                                     << clef_shm
                        ...
                        assert((hSharedSeg = OpenFileMapping(
                                  FILE_MAP_ALL_ACCESS, FALSE, pcClefNum)) !=
                               INVALID_HANDLE_VALUE);

                        assert((lpSharedSeg = MapViewOfFile(
                                  hSharedSeg,
                                  FILE_MAP_ALL_ACCESS,
                                  0,
                                  0,
                ...
                for (i = 0; i < nbf; i++) {
                        // Geston de la sortie Etrivec0 identique a liste de
                        // triangle en entree - MC09
                        ...
                        if (!(sol && diff->primi().name() == 0)) {
                                ...
                                switch (opak) {
                                        case 0: // Opaque
                                                ...
                                                ia++;
                                                break;
                                        case 1: // Transparent[face sup] =>
                                                // preparation
                                                r0 = diff->rho();
                                                t0 = diff->tau();
                                                break;
                                        case 2: // Transparent[face inf] =>
                                                // resolution du syst 2eq, 2inc
                                                // => E0 et E1
                                                r1 = diff->rho();
                                                t1 = diff->tau();
                                                D = r0 * r1 - t0 * t1;
                                                ...
                                                if (byfile) {
                                                        ...
                                                        // MCoct05: caribu4.4
                                                        /* Bug 221105 MC
                                                           Te[ia+(Nt+1)]=B[0]->ve[i-1]*surf;//face
                                                           sup
                                                           Te[ia+2*(Nt+1)]=B[0]->ve[i]*surf;
                                                           //face inf
                                                        */
                                                        Te[ia + (Nt - 1)] =
                                                          Ei[i - 1] *
                ...
                */
                ...
                if (byfile) {
                        ...
                        // Complicated Way
                        UnmapViewOfFile(
                          lpSharedSeg);  // invalidation du ptr sur mem partagee
...
//======>  beep(): fait bip !
inline void
beep(const char* msg = "M'enfin ...", int nbeep = 1)
{
...
erreur_syntaxe(char* prog)
{
        ...
        Ferr << "  -M filename \t File describing the scene\n"
                ...
                "  -s Ns\t Append a soil to the scene (Ns is a treshold for "
                ...
                "  -B \t\t Test the effect of the choice of inner triangles "
                "(bias?) \n"
                ...
                "  -w filename\t Read the matrix file to simulate an other "
                "radiative case, without to compute form factors \n"
                "  -t dirname \t Name of the directory where the FF file is "
                "stored\n"
#endif
                "  -h \t\t This help message "
             << '\n'; // "%c",7);
...
options(int argc, char** argv)
{
        int c;
        GetOpt option(
          argc, argv, "AC:BFTg1hs:L:M:R:S:8:a:d:e:f:i:l:m:n:p:r:t:v:w:");
        ...
        nbsim = 1;
        denv = 0.30;
        seuil = 1e-6; //-1 ie seuil_solver=MACHEPS
        ffseul = infty = geom = ordre1 = ff_print = bio = byseg = byfile =
          radonly = memsize = solem = false;
        bias = true;
        lightname = maqname = envname = optname = name8 = dirname = matname =
          nsolem = NULL;
        sol = 0;
        scene.Timg = 1536;
        // Traitememnt des options
        if (argc < 2) {
                erreur_syntaxe(argv[0]);
                return 1;
        }
        while ((c = option()) != EOF)
                switch (c) {
                        case 'A':
                                bio = true;
                                break; // genere Eabs.dat et Einc.dat
                        case 'B':
                                bias = false;
                                break; // pb des a cheval sur la sphere
                        case 'C':
                                nsolem = option.optarg;
                                solem = true;
                                break; // solem.can
                        case 'F':
                                ff_print = true;
                                break; // FF -> FF.dat
                        case 'L':
                                scene.Timg = atoi(option.optarg);
                                break; // Resolution projplan
                        case 'M':
                                maqname = option.optarg;
                                byfile = true;
                                break; // maquette .can
                        case 'S':
                                nbsim = atoi(option.optarg);
                                break; // nombre de simulations
                        case 'R':
                                NB = atoi(option.optarg);
                                break; // Resolution FF
                        case 'T':
                                memsize = true;
                                break; // Appel maxmem> maxmem.res mem en Ko
                        case '1':
                                ordre1 = true;
                                break; // stop apres ordre 1
                        case '8':
                                infty = true;
                                name8 = option.optarg;
                                break; // infinity
                        case 'a':
                                seuil = atof(option.optarg);
                                break; // seuil de convergence
                        case 'd':
                                denv = atof(option.optarg) / 2.;
                                break; // diam de la sphere
                        case 'e':
                                envname = option.optarg;
                                break; // donnees de l'envt (eg. sail)
                        case 'f':
                                matname = option.optarg;
                                radonly = false;
                        ...
                        case 'h':
                                ...
                                return 1;
                        case 'i':
                                nb_iter = atoi(option.optarg);
                                break; // nbre d'iterations
                        case 'l':
                                lightname = option.optarg;
                                break;
                        case 'm':
                                clef_shm = atoi(option.optarg);
                                byseg = true;
                                break; // by segmem clef
                        case 'p':
                                optname = option.optarg;
                                break;
                        case 'r':
                                denv = atof(option.optarg);
                                break; // rayon de la sphere
                        case 's':
                                sol = atoi(option.optarg);
                                ;
                                break; // ajoute un sol
                        case 't':
                                dirname = option.optarg;
                                break; // specifie le dir des hd mat  ; defaut =
                                       // /tmp
                        case 'v':
                                verbose = (char)atoi(option.optarg);
                                break; // verbose
                        case 'w':
                                matname = option.optarg;
                                radonly = true;
                                bMemoriseMatrix = true;
                                break;
                                // Without :  lit la matrice pour refaire des
                                // calculs en changeant les po ou le sun
                        default:
                                erreur_syntaxe(argv[0]);
                                return 1;
                } // switch
        ...
        if (clef_shm != -1) {
                Ferr << "-------------o clef_shm = " << clef_shm
                     << " o---------------" << '\n';
        }
        if (byseg && byfile) {
                Ferr << "<!> Fatal error" << (char)7
                     << "\n==> Canestra should be called "
                        "with 3 necessary options:\n -M maqname -p optname -l "
                        "lightname \n ";
                return 1;
        }
        if (((maqname == NULL) && byfile) || ((clef_shm == -1) && byseg) ||
            (lightname == NULL) || (optname == NULL)) {
                Ferr
                  << "<!> Fatal error" << (char)7
                  << "\n==> Canestra should be called "
                     "with 3 necessary options:\n"
        ...
        if (sol) {
                ...
                if (ordre1) {
                        sol = 0;
                        Ferr << "<!> l'option \"1er ordre\" desactive l'option "
                                "\"sol\" \n";
                }
        }
        if (infty) {
                ...
                if (denv < 0) {
                        Ferr
                          << "<!> Fatal error " << (char)7 << '\n'
                          << "==> Canestra is called with 2 incompatible "
                             "options:\n"
        ...
        if (!bias)
                Ferr << " Option test du biais(?) des triangles a-cheval sur "
                        "la sphere ACTIVEE\n";
        if (envname == NULL && !ordre1) {
                ...
                if (denv == 0) {
                        ...
                        Ferr << "<!> Fatal error" << '\n'
                             << "==> Canestra is called with 2 incompatible "
                                "options:\n"
                             << "sphere diameter null (-d or -r) and no mean "
                                "fluxes files (-e)"
                             << '\n';
                        return 1;
                }
        ...
        if (memsize) {
                char cmd[125];
                snprintf(
                  cmd, sizeof(cmd), "maxmem %s 1 > maxmem.res &", argv[0]);
                if (verbose)
                        Ferr << "Option -T :  Pour avoir le max de memoire "
                                "occupee = "
                             << cmd << "\n";
                if (system(cmd) == -1) {
