...
class Canopy
{
      ...
      public:
        ...
        ListeD<double> Ldiff0; // liste des labels des diffuseurs du .can (bon
                               // et pas bons) - MC10
        int Timg; // Resolution de l'image projplan (Avant en #define) - 0699
                  // (default 1536)
        // member function
        ...
        long int
        parse_can(char*, char*, char*, reel*, reel*, int, char*, Diffuseur**&);
        long int
        read_shm(int, char*, char*, reel*, reel*, int, char*, Diffuseur**&);
        void cstruit_grille(double Renv)
        {
                mesh.construction(bmin, bmax, Renv, Ldiff);
        }
        void sail_pur(VEC** Cfar, double* Esource, char* envname);

#ifdef _HD
        void calc_FF_Bfar(VEC** Cfar,
                          double* Esource,
                          char* envname,
                          bool bias,
                          double denv = 0.3,
