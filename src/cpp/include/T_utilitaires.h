...
#include <stdarg.h>
//#include
//"/usr/local/sparc/lib/gcc-lib/sparc-sun-solaris2.5.1/2.7.2.2.f.2/include/stdarg.h"
...
class Tabdyn
{
      ...
      public:
        ...
        {
                if (D > 1) {
                        Ferr << "Tabdyn[quicksort] implemente que pour les "
                                "tableaux 1D"
                             << '\n';
                        exit(34);
                }
                if (!trie) {
                        ...
                        qsort(tab,
                              taille,
                              sizeof(Type),
                              (int (*)(const void*, const void*))clef);
                }
        }
...
template<class Type, unsigned int D>
void
Tabdyn<Type, D>::maj(Type val)
{
        if (tab == NULL) {
                Ferr << "\nTabdyn [maj()]: Tableau dynamique pas alloue: acces "
                        "impossible!";
                Ferr << '\n';
                exit(36);
        }
        ...
        Type* pt;
        trie = false;
        for (i = 0, pt = tab; i < taille; i++, pt++)
                *pt = val;
} // maj()
//*********** bonind() *****************
template<class Type, unsigned int D>
inline int
Tabdyn<Type, D>::bonind(int ind, unsigned char indmax)
{
        if (ind < 0 || ind >= max[indmax]) {
                Ferr << "\n Tabdyn[operator()]: indice no " << ind
                     << " hors-borne!" << '\n';
                ind = (int)(1 / sin(0.0));
                exit(37);
...
template<class Type, unsigned int D>
inline int
Tabdyn<Type, D>::bonmax(int ind)
{
        if (ind <= 0) {
                Ferr << "\n Tabdyn[bornmax()]: dimension de tableau negative "
                        "ou nulle!"
                     << '\n';
                exit(38);
                return ind; // pour eviter les warning verbeux de SGI
...
template<class Type, unsigned int D>
inline void
Tabdyn<Type, D>::lectarg(int& first, va_list& ptarg)
{
...
template<class Type, unsigned int D>
void
Tabdyn<Type, D>::alloue(int first, ...)
{
        if (tab != NULL) {
                Ferr << "\nTabdyn [alloue()]: Tableau dynamique deja alloue a "
                        "la declaration!"
                     << '\n';
                exit(40);
        }
...
template<class Type, unsigned int D>
Type&
Tabdyn<Type, D>::operator()(int first)
{
        if (D == 1)
                return (tab[first]);
        else {
                Ferr << "Tabdyn<Type,D>::operator()(int first) incompatbile "
                        "avec D=";
                Ferr << D << '\n';
                exit(41);
...
template<class Type, unsigned int D>
Type&
Tabdyn<Type, D>::operator()(int first, int second)
{
        if (D == 2)
                return (tab[first + max[0] * second]);
        else {
                Ferr << "Tabdyn<Type,D>::operator()(int first,int second) "
                        "incompatbile";
                Ferr << " avec D=" << D << '\n';
                exit(42);
                return (tab[0]); // pour eviter les wranings verbeux de SGI !!!
...
template<class Type, unsigned int D>
Type&
Tabdyn<Type, D>::operator()(int first, int second, int third, ...)
{
        /*  if(tab==NULL) {
          Ferr<< "Tabdyn [operator()]: Tableau dynamique pas alloue : acces
        impossible!"<<'\n'; exit(43);
        }
        */
        va_list ptarg;
...
class Noeud
{
      ...
      public:
        ...
        inline void next(Noeud<Type>* n); // initialise le pointeur sur suivant
                                          // du noeud a la valeur n
...
class Liste
{
      ...
      public:
        ...
        inline void
        suivant(); // met le pointeur courant sur l'element suivant par rapport
        ...
        inline void
        detruire_droite(); // detruit l'element a droite de l'element courant
...
class NoeudD
{
      ...
      public:
        ...
        inline void next(NoeudD<Type>* n); // initialise le pointeur sur suivant
                                           // du noeud a la valeur n
        inline NoeudD<Type>* back(); // renvoie le pointeur sur prec du noeud
        inline void back(NoeudD<Type>* n); // initialise le pointeur sur prec du
                                           // noeud a la valeur n
...
class ListeD
{
      ...
      public:
        ...
        inline void suivant(); // met le pointeur courant sur l'element suivant
                               // par rapport  a la position courante
        inline void
        precedent(); // met le pointeur courant sur l'element precedent par
                     // rapport  a la position courante
        inline Type& contenu(); // renvoie le contenu d'un element de la liste
        inline void detruire(); // detruit l'element courant
        inline void
        detruire_droite(); // detruit l'element a droite de l'element courant
      ...
      private:
        void qsort(NoeudD<Type>*,
                   NoeudD<Type>*,
                   int (*)(Type,
                           Type)); // Fonction recursive utilisee par Quicksort
        inline void invert(
          NoeudD<Type>*,
          NoeudD<Type>*); // intervertit deux elements de la liste
};                        // class ListeD

template<class Type>
inline void
detruire_contenu(ListeD<Type*>&);
...
template<class Type>
inline void
Liste<Type>::ajoute(Type contenu)
{
        if (est_vide() == VRAI)
                init_liste(contenu);
        else
...
template<class Type>
inline void
Liste<Type>::init_liste(Type contenu)
{
...
template<class Type>
inline void
Liste<Type>::ajout_elem(Type contenu)
{
...
{ // fucke', revu MC9/10/96
        ...
        if (!est_vide()) {
                ...
                if (courant == premier) {
                        ...
                        for (prec = premier; prec->next() != courant;
                             prec = prec->next())
                                ;
                        prec->next(courant->next());
                        // cout << "destruction du courant" << courant << endl;
                        // cout.flush();
                        delete courant;
                        courant = prec;
                        nbe--;
                ...
                Ferr << "ERREUR - Impossible de detruire un element car liste "
                        "deja vide";
                Ferr << '\n';
...
Liste<Type>::detruire_droite()
{
        ...
        if (!est_vide()) {
                if (!est_fin()) {
                        ...
                        Ferr << "ERREUR - Impossible de detruire un element "
                                "car fin de liste"
                             << '\n';
                        exit(44);
                }
                ...
                Ferr
                  << "ERREUR - Impossible de detruire un element car liste vide"
                  << '\n';
                exit(45);
        }
}
...
template<class Type>
void
Liste<Type>::ranger(Type element)
{
        if (est_vide() == VRAI)
                init_liste(element);
        else
...
template<class Type>
inline void
detruire_contenu(Liste<Type*>& liste)
{
        Type* donnee;
        liste.debut();
        while (!liste.est_fin()) {
...
template<class Type>
inline void
NoeudD<Type>::next(NoeudD<Type>* n)
{
        suivant = n;
}
...
template<class Type>
inline void
NoeudD<Type>::back(NoeudD<Type>* n)
{
        prec = n;
}
template<class Type>
inline NoeudD<Type>&
NoeudD<Type>::operator=(NoeudD<Type>& rval)
{
        donnee = rval.donnee;
...
template<class Type>
inline void
ListeD<Type>::ajoute(Type contenu)
{
        if (est_vide() == true)
                init_liste(contenu);
        else
...
template<class Type>
inline void
ListeD<Type>::init_liste(Type contenu)
{
...
template<class Type>
inline void
ListeD<Type>::ajout_elem(Type contenu)
{
...
ListeD<Type>::detruire_droite()
{
        ...
        if (!est_vide()) {
                if (!est_fin()) {
                        ...
                        Ferr << " ListeD[detruire_droite]  Impossible de "
                                "detruire un element car fin de liste"
                             << '\n';
        } else
                Ferr << "ListeD[detruire_droite] Impossible de detruire un "
                        "element car liste vide"
                     << '\n';
} // ListeD::detruire_droite()

template<class Type>
inline void
ListeD<Type>::free_liste()
{
        if (!est_vide())
                for (debut(); !est_vide(); detruire())
                        ;
...
template<class Type>
inline void
ListeD<Type>::invert(NoeudD<Type>* A, NoeudD<Type>* B)
{
        Type tmp;
        tmp = A->donne();
        A->donne(B->donne());
...
template<class Type>
void
ListeD<Type>::qsort(NoeudD<Type>* g,
                    NoeudD<Type>* d,
...
template<class Type>
void
ListeD<Type>::QuickSort(int (*comp)(Type x, Type y))
{
        NoeudD<Type>*g, *d;
...
template<class Type>
inline void
detruire_contenu(ListeD<Type*>& liste)
{
        Type* donnee;
        liste.debut();
        while (!liste.est_fin()) {
