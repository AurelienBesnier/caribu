...
class Homogene
{
      ...
      public:
        Homogene(const reel a = 0,
                 const reel b = 0,
                 const reel c = 0,
                 const reel d = 0);
        // OC  Homogene (const Homogene&);  // constructeur par copie // vire
        // par OC le 29 4 98
        ...
        // OC  Homogene& operator = ( const Homogene&); // vire par OC le 29 4
        // 98
        Homogene operator*(
          const double&) const; // multiplication par un scalaire
        Homogene& operator*=(const double&);
        Homogene operator/(const double&) const; // division par un scalaire
        Homogene& operator/=(const double&);
        //  Homogene operator * ( class Matrice4&); //multiplication par une
        //  matrice
        Homogene chgt_base(const Vecteur& u,
                           const Vecteur& v,
                           const Vecteur& w) const;
        void show() const
        {
                Ferr << " Homogene : " << homo[0] << "  " << homo[1] << "  "
                     << homo[2] << "  " << homo[3] << '\n';
        }
};
...
class Point : public Homogene
{
      public:
        ...
        Point operator+(
          const Homogene&) const; // addition d'un point et d'un vecteur
        Point operator-(
          const Homogene&) const; // soustraction d'un point et d'un vecteur
        ...
        {
                return sqrt(((homo[0] - A[0]) * (homo[0] - A[0])) +
                            ((homo[1] - A[1]) * (homo[1] - A[1])) +
                            ((homo[2] - A[2]) * (homo[2] - A[2])));
        }
        double dist2(const Point& A) const
        {
                return ((homo[0] - A[0]) * (homo[0] - A[0])) +
                       ((homo[1] - A[1]) * (homo[1] - A[1])) +
                       ((homo[2] - A[2]) * (homo[2] - A[2]));
        }
};
...
class Vecteur : public Homogene
{
      public:
        ...
        Vecteur operator-(
          const Vecteur&) const; // soustraction de deux vecteurs
        Vecteur& operator-=(const Vecteur&);
        ...
        double prod_mixte(const Vecteur&,
                          const Vecteur&) const; // produit mixte
...
inline Homogene::Homogene(const reel a,
                          const reel b,
                          const reel c,
                          const reel d)
{
...
inline Homogene
Homogene::operator*(const double& a) const
{
        Homogene res;
...
inline Homogene&
Homogene::operator*=(const double& a)
{
        homo[0] *= a;
        homo[1] *= a;
        homo[2] *= a;
...
template<class Type>
Type&
Matrice<Type>::operator()(int i, int j)
{
        if (i < n && j < m)
                return (M[m * i + j]);
        else {
...
template<class Type>
Matrice<Type>&
Matrice<Type>::operator=(Matrice<Type>& A)
{
        int i, j;

        if (n != A.n || m != A.m) {
                Ferr << "ERREUR - Impossible d'egaliser 2 matrices de taille "
                        "differente"
                     << '\n';
                cout.flush();
                exit(32);
