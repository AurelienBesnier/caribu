...
class Diffuseur
{
      ...
      public:
        ...
        Diffuseur()
        { // cout<<"Diffuseur [constructeur] Warning : no parameters!\n";
        }
        Diffuseur(Primitive* pprim, Actop* actop)
          : prim(pprim)
          , opti(actop)
        {
        }
        ...
        // amie
        friend int maxE(Diffuseur*,
                        Diffuseur*); // utilise par QuickSort (TabDyn, ListeD)
        // renvoie -1 si E1>E2, 0 si E1=E2, 1 si E1<E2 (Ei delta energie du
        // difuseur i
...
class DiffP : public DiffO
{ // Capteur virtuel

      public:
        DiffP(Primitive* p, Actop* actop)
          : DiffO(p, actop)
        {
        }
        DiffP(Primitive* p)
...
class DiffT : public Diffuseur
{
      ...
      public:
        ...
        void active(unsigned char cefa) { actif = (cefa == 1) ? inf : sup; }
        void activ_num(unsigned int nummer)
        {
                if (nummer != num()) {
                        togle_face();
                        if (nummer != num()) {
                                Ferr << " Stronzo faux nummer dans "
                                        "DiffT::activ_num"
                                     << '\n';
                                exit(30);
                        }
