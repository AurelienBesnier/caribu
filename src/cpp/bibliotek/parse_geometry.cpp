...
#define PITEM                                                                  \
        6 /*x,y,z,Nx,Ny,Nz*/ /* [11] size of one vertex defini dans            \
                                cpfg:control.h*/
#define NORMAL 1
#define NORMAL_X 3
#define NORMAL_Y 4
...
/* draws triangles */
void
OGLPrim::DrawTriangle(float* p1, float* p2, float* p3)
{
        short i;
        ...
        if (surf < 1e-8) {
                ...
                Ferr << ">>> parse_geometry.cpp:OGLPrim::DrawTriangle() : "
                        "elimination d'un triangle aberrant..."
                     << '\n';
                ...
                for (i = 0; i < 3; i++)
                        G[i] += Gt[i] * surf;
                nbT++;
                // if(1)fprintf(stderr," surf=%f,G(%.4f,%.4f,%.4f) =>
...
void
OGLPrim::DrawRectangle(float* p1, float* p2, float* p3, float* p4)
{
        ...
        switch (primitive.flag) {
                ...
                case MESH:
                        if (primitive.num_items >= 4 * 3) {
                                for (j = 0; j <= primitive.num_items - 4 * 3;
                                     j += 2 * 3) {
                                ...
                                Ferr << "Caribu<prim2triangle> At least four "
                ...
                case POLYGON:
                        if (primitive.num_items >= 9) {
                                for (j = 0; j < 3; j++) {
                                        vec1[j] = primitive.data[j] -
                                                  primitive.data[3 + j];
                                        vec2[j] = primitive.data[6 + j] -
                                                  primitive.data[3 + j];
                                }
                                /* get the normal */
                                norm[0] = vec1[1] * vec2[2] - vec1[2] * vec2[1];
                                ...
                                */
                                ...
                                Transform3Point(
                                  &primitive.data[0], rotmat, pt[0]);
                                Transform3Point(
                                  &primitive.data[3], rotmat, pt[1]);
                                Transform3Point(
                                  &primitive.data[6], rotmat, pt[2]);

                                DrawTriangle(pt[0], pt[1], pt[2]);

                                for (j = 9; j <= primitive.num_items - 3;
                                     j += 3) {
                                        for (c = 0; c < 3; c++) {
                                                pt[1][c] = pt[2][c];
                                        }
                                        Transform3Point(
                                          &primitive.data[j], rotmat, pt[2]);
                                ...
                                // fprintf(stderr,"POLYGON P2x-P1x: anant=%.3f,
                                // apres=%.3f\n",vec1[0],vec2[0]);

                        } else
                                Ferr << "POLYGON P2x-P1x: anant=" << vec1[0]
                                     << ", apres=" << vec2[0] << "\n"
                                     << "Caribu<prim2triangle>Polygon At least "
                                        "three points"
                ...
                case POLYGONUV:
                        if (primitive.num_items >= 9 * 2) {
                                ...
                                Transform3Point(
                                  &primitive.data[0], rotmat, pt[0]);
                                Transform3Vector(
                                  &primitive.data[3], rotmat, pt[0] + 3);
                                Transform3Point(
                                  &primitive.data[6], rotmat, pt[1]);
                                Transform3Vector(
                                  &primitive.data[9], rotmat, pt[1] + 3);
                                Transform3Point(
                                  &primitive.data[12], rotmat, pt[2]);
                                Transform3Vector(
                                  &primitive.data[15], rotmat, pt[2] + 3);
                               ...
                               */
                                ...
                                for (j = 18; j <= primitive.num_items - 6;
                                     j += 6) {
                                        for (c = 0; c < 6; c++) {
                                                pt[1][c] = pt[2][c];
                                        }
                                        Transform3Point(
                                          &primitive.data[j], rotmat, pt[2]);
                                        Transform3Point(&primitive.data[j + 3],
                                                        rotmat,
                                                        pt[2] + 3);
                                ...
                                Ferr << "Caribu<prim2triangle> At least three "
                                        "points with"
                                     << " normals must be specified for "
                                        "polygonuv\n";
                        break;
                case RECTANGLE:
                        if (primitive.num_items >= 2) {
                                ...
                                Ferr << "At least two values must be specified "
                                        "for rectangle.\n";
                        break;
                ...
                case PUSHMATRIX:
                        ...
                        if (primitive.num_items >= 1)
                                Ferr
                                  << "Caribu<prim2triangle> Warning: "
                                     "pushmatrix doesn't need a parameter.\n";
                        break;
                case POPMATRIX:
                        PopMatrix();
                        if (primitive.num_items >= 1)
                ...
                case TRANSLATE:
                        if (primitive.num_items >= 3)
                                ...
                                Ferr << "Caribu<prim2triangle> Warning: not "
                                        "enough parameters for translate!\n";
                        break;
                case ROTATE:
                        if (primitive.num_items >= 4)
                ...
                case MULTMATRIX:
                        ...
                        break;
                case SPHERE:
                        Ferr << "Caribu<prim2triangle> sphere not yet "
                                "implemeted\n";
                        /* if(primitive.num_items >=1) {
                           ...
                           Ferr <<"Caribu<prim2triangle> Warning: not enough
                           parameters for sphere!\n" ;
                        */
                        break;
                case CYLINDER:
                        Ferr << "Caribu<prim2triangle> cylinder not yet "
                                "implemeted\n";
                        /*if(primitive.num_items >=2) {
                          SetTurtle(&dummy_turtle);
                          ...
                          Translate(translate);
                          SetTurtle(&dummy_turtle);
                          (*dr->tdd->EndNode)(&dummy_turtle, dr,
                          &viewparam,'\0'); Rotate(rotate90);
                          SetTurtle(&dummy_turtle);
                          (*dr->tdd->Circle3D)(&dummy_turtle, dr, &viewparam,
                          primitive.data[0]*2);
                          PopMatrix();
                ...
                case CONE:
                        Ferr
                          << "Caribu<prim2triangle> cone not yet implemeted\n";
                        /* if(primitive.num_items >=3) {
                           SetTurtle(&dummy_turtle);
                ...
                case PRISM:
                        Ferr
                          << "Caribu<prim2triangle> prism not yet implemeted\n";
                        /* if(primitive.num_items >= 3) {
                           for(x=0;x<2;x++)
                           for(y=0;y<2;y++)
                           for(z=0;z<2;z++) {
                           ...
                           DrawRectangle(P[0][0][0], P[0][0][1],  P[0][1][1],
                           P[0][1][0]); DrawRectangle(P[1][0][0], P[1][1][0],
                           P[1][1][1], P[1][0][1]);

                           DrawRectangle(P[0][0][0], P[0][1][0],  P[1][1][0],
                           P[1][0][0]); DrawRectangle(P[0][0][1], P[1][0][1],
                           P[1][1][1], P[0][1][1]);

                           DrawRectangle(P[0][0][0], P[0][0][1],  P[1][0][1],
                           P[1][0][0]); DrawRectangle(P[0][1][0], P[1][1][0],
                           P[1][1][1], P[0][1][1]);
                           }
                           else
                           Ferr <<"Caribu<prim2triangle> Warning: not enough
                           parameterss for prism!\n" ;
                        */
                        break;
                default:
                        Ferr << "Caribu<prim2triangle> Warning: unknown "
                                "primitive.\n";
        }
...
/*************************************************************************/
int
OGLPrim::parse_line(char* line)
{
        // line <= CSGetString(), T[nbT] return the triangles describing the
        // analysed primitive
        ...
        if (!isalpha(line[0])) {
                ...
                if (analysing) { // all the parameters are stored => dealing the
                                 // corresponding primitive
                        answer = prim2triangle();
                        empty = 1;
                        // fprintf(stderr,"parse_geom_line: apre prim2T
                        // emty=%d", empty);
                }
                ...
                while (ptr->flag != -1) {
                        ...
                        if (strncmp(line, ptr->keyword, lstr) == 0) {
                                ...
                                str = line + lstr;
                                // fprintf(stderr,"OGLPrim.parse_line:
                                // keyword=%s\n",ptr->keyword);
                                analysing = 1;
                                break; /* out of the loop going through all
                                          keywords */
                        }
                        ptr++;
                }
        ...
        if (analysing) {
                ...
                for (i = 0;; i++) {
                        ...
                        if (isspace(c) || c == 0) {
                                if (record) {
                                        real[ir] = 0;
                                        // fprintf(stderr,"parse_geom_str:
                                        // data[num_item=%d]=",primitive.num_items);
