...
GetAllFileName(char* nom)
{
        ...
        if (true) {
                ...
                // I find out %TEMP% by myself  Ferr << __FILE__ " : "<<
                // __LINE__ << '\n' ;
                ...
                if (iTest != 0) {
                        cerr << __FILE__ << ": Pas pu executer " << ccCommande
                             << endl;
                        exit(20);
                };
                ...
                //  Attention : XP ne renvoie rien si une variable n'est pas
                //  d�finie (style posix) HA 07 2004
                if ((sTmpName[0] == '%') || (sTmpName.size() == 0)) {
                        istringstream isIn(".");
                        isIn >> sTmpName;
                        clog << "No %TEMP% environmental found.\n";
                        clog << "From now on the temp dir is going to be .\\."
                             << endl;
                }
                fin.close();
