...
void
Polygone::init(Liste<Point>& liste_sommet, double name = 0)
{
        int i = 1;

        nom = name;
        for (liste_sommet.debut(); !liste_sommet.est_fin();
...
Polygone::Polygone(char* line, double name, reel* mini, reel* maxi, bool valid)
{
...
Polygone::Polygone(string ligne,
                   double name,
                   reel* mini,
                   reel* maxi,
                   bool valid)
{
        ...
        char *tmp1 = new char[LONG_LIGNE_CAN], *tmp2 = new char[LONG_LIGNE_CAN];
        strcpy(tmp2, "");
...
//-*******************   Polygone::init(char *,...)   ***********************
void
Polygone::init(char* line, double name, reel* mini, reel* maxi, bool valid)
{
                ...
                if (nb_sommets >= 3) {
                        ...
                        if (fabs(u.prod_scalaire(v)) > 0.9999999999) {
                                Ferr << "\tPolygone::init(char*) => "
                                        "Comme les sommets 0, 1 et  2  sont "
                                        "aligne's, \n";
                                if (bb) {
                                        mini[0] = 1;
                                        maxi[0] = 0;
                        ...
                        x = v.norme();
                        if (x <= 0) {
                                Ferr << "\tPolygone::init(char*) => Norme de "
                                        "(POP1)^(POP2 Nulle \n";
                                if (bb) {
                                        mini[0] = 1;
                                        maxi[0] = 0;
...
Polygone::Polygone(float (*T)[3], double name, reel* mini, reel* maxi)
{
        ...
        if (nb_sommets >= 3) {
                ...
                x = u.norme();
                if (x == 0.) {
                        Ferr << "\tPolygone(reel **) => Comme les sommets 0 et "
                                "1 sont egaux, \n";
                        mini[0] = 1;
                        maxi[0] = 0;
                        return;
                ...
                v /= x;
                if (sommet[1] == sommet[2]) {
                        Ferr << "\tPolygone(reel **) => Comme les sommets 1 et "
                                "2 sont egaux, \n";
                        mini[0] = 1;
                        maxi[0] = 0;
                        return;
                ...
                if (fabs(u.prod_scalaire(v)) > 0.9999999999) {
                        Ferr << "\tPolygone(reel **) => Comme les sommets 0, 1 "
                                "et  2  sont aligne's, \n";
                        mini[0] = 1;
                        maxi[0] = 0;
                        return;
                ...
                x = v.norme();
                if (x <= 0) {
                        Ferr << "\tPolygone(reel **) => Norme de (POP1)^(POP2 "
                                "Nulle \n";
                        mini[0] = 1;
                        maxi[0] = 0;
                        return;
...
void
Polygone::show(const char* msg, ostream& out)
{
        out << msg << "-Polygone :";
        qui();
        for (int i = 0; i < nb_sommets; i++) {
                out << "\t" << sommet[i][0] << " " << sommet[i][1] << " "
                    << sommet[i][2] << endl;
        }
} // show()
...
int
Polygone::tout_point_inf(reel& position, const int& axe)
{
        int i, compris = 1;
...
int
Polygone::tout_point_sup(reel& position, const int& axe)
{
        int i, compris = 1;
...
int
Polygone::nb_in(reel& amin, reel& amax, const int& axe)
{
        int nbp = 0, i;
        bool info = false;
        for (i = 0; i < nb_sommets; i++) {
                if (sommet[i][axe] > amax) {
                        if (nbp < 0) {
                                Ferr
                                  << " (!) Pb Polygone::nb_in : primitive no. "
                                  << nom << " plus grande que les bornes \n";
                                info = true;
                        }
                        nbp++;
                } else if (sommet[i][axe] < amin) {
                        if (nbp > 0) {
                                Ferr
                                  << " (!) Pb Polygone::nb_in : primitive No. "
                                  << nom << " plus grande que les bornes\n!";
...
void
Polygone::calcul_normale_cst_equ(Point& p1, Point& p2, Point& p3)
{
        ...
        // Calcul de l'isobarycentre et de la + grde distance de isob aux
        // sommets (aprt_sphere)
        int i, j;
        float d2;
...
// TRIANGLE
Triangle::Triangle(Liste<Point>& liste_sommet, double name = 0)
  : Polygone(liste_sommet, name)
{
        if (nb_sommets != 3) {
                Ferr
                  << "ERREUR - nombre de sommets incoherent pour un triangle\n";
...
Polygone::dedans(const Point& I)
{
        ...
        i = 0;
        if (fabs(normale[1]) > fabs(normale[0]))
                i = 1;
        if (fabs(normale[2]) > fabs(normale[1]))
                i = 2;
        j = (i + 1) % 3;
        k = (j + 1) % 3;
...
double
Polygone::intersect(Param_Inter& parag, Point* I)
{
        // algo de SNYDER
        ...
        *I = parag.origin() + parag.direct() * t;
        // cout<<"Polygone[intersect] inter I : "<<(*I)[0]<<"  "<<(*I)[1]<<"
        // "<<(*I)[2]; qui();
        //  Le point appartient-il au polygone ?
        if (dedans(*I)) {
                // cout<<"Polygone[intersect] dedans I : "<<(*I)[0]<<"
                // "<<(*I)[1]<<"  "<<(*I)[2];qui();
                return t;
        } else
                return 4e9;
...
Polygone::surface()
{
        ...
        switch (nb_sommets) {
                ...
                case 2:
                        Ferr << "Polygone[surface] Irrtum nb_sommet = "
                             << nb_sommets << '\n';
                        exit(26);
                        break;
                case 3: // triangle
                        ...
                        x = sommet[0].dist(sommet[1]) *
                            sommet[0].dist(sommet[2]) * sin(x) / 2.0;
                        raus(x < 0.0,
                             "Polygone[surface] surface < 0.0 !!??"); // warning
                                                                      // trigraphe
                                                                      // (??.)
                        break;
                case 4:
...
Triangle::surface()
{
        ...
        x =
          sommet[0].dist(sommet[1]) * sommet[0].dist(sommet[2]) * sin(x) / 2.0;
...
double
Polygone::distance2_point(Point& C)
{
        /* Calcule le carre de la distance d'un point a un polygone convexe T en
           2 etapes :
           - I, intesection du plan de T avec la droite (C,n), o� n est la
           normale de T (cf. intersect())
           - Determination de I', point minimisant la distance (T,I) dans le
           plan
           */
        ...
        if (verbeux > 1)
                printf(" cst_eq_plan = %g -- t = %g\n", cst_equ_plan, t);
        I = C + normale * t;
        if (verbeux > 1)
                cout << "Polygone[distance_point] inter I : " << I[0] << " "
                     << I[1] << " " << I[2] << endl;
        ...
        for (inc = 0; inc < nb_sommets; inc++) {
                ...
                if (signG[inc])
                        idx[ii++] = inc;
                if (verbeux)
                        printf("sG(%d)=%d - ", inc, (int)signG[inc]);
        }
        if (verbeux < 0) {
                time(&tpsF);
                tps1 += tpsF - tpsD;
        ...
        switch (ii) {
                ...
                case 1: // printf("ii=1\n");
                        ...
                        v.formation_vecteur(sommet[idx[0]],
                                            sommet[(idx[0] + 1) % nb_sommets]);
                        scal = u.prod_scalaire(v);
                        if (scal <= 0) {
                                if (verbeux)
                                        printf("1.1 dist(C,S(%d))\n", idx[0]);
                                d2 = C.dist2(sommet[idx[0]]);
                        } else {
                                u.formation_vecteur(
                                  sommet[(idx[0] + 1) % nb_sommets], I);
                                v = -v;
                                scal = u.prod_scalaire(v);
                                if (scal <= 0) {
                                        if (verbeux)
                                                printf("1.2 dist(C,S(%d))\n",
                                                       (idx[0] + 1) %
                                                         nb_sommets);
                                        d2 = C.dist2(
                                          sommet[(idx[0] + 1) % nb_sommets]);
                                        ...
                                        // Cast oblige le 310798 avec
                                        // egcs 2.91.52....
                                        I = Point(sommet[idx[0]] * alpha) +
                                            sommet[(idx[0] + 1) % nb_sommets] *
                                              (1 - alpha);
                                        if (verbeux) {
                                                printf("1.3 dist(C,S%dS%d))\n",
                                                       idx[0],
                                                       (idx[0] + 1) %
                                                         nb_sommets);
                                                I.show();
                ...
                case 2:
                        ...
                        if (scal >= 0) {
                                alpha = scal / v.prod_scalaire(v);
                                I = Point(sommet[idx[1]] * alpha) +
                                    sommet[idx[0]] * (1 - alpha);
                                if (verbeux) {
                                        printf("2.1 dist(C,S%dS%d))\n",
                                               idx[1],
                                               idx[0]);
                                        I.show();
                                }
...
bool
Polygone::appart_sphere(Point& O, double R)
{
        // sous-estimateur de la distance de O au poly par efficacite
        double d;

        d = isob.dist(O) - dGS;
        // printf("appart_sphere()  d=%g (dOG=%g,
