...
/*         glProj.cpp
           ...
           la couleur des faces des triangles etant d�finie par un indice
   entierunique pour chaque face de  triangle
*/
/**********************************************************************/
//         MC09
/**********************************************************************/
// compile:
// MacOSX
//  g++  -framework GLUT -framework OpenGL -Wall -o glproj glProj.cpp
//  -L"/System/Library/Frameworks/OpenGL.framework/Libraries" -lGL -lGLU -lm
//  -lobjc -lstdc++
...
// todo: tester chaargement de la scene avnt le gluLookat et de chger le
// gluLookAt ss recharger la scene : DONE grace au Display list to do encapsuler
// le taf OpenGL dans une classe: DONE Todo: mettre les couleur de la lut et
// compter les occurences des idx de la LUT: done
...
class Pixel
{
      public:
        ...
        Pixel() {}
        Pixel(float r, float g, float b) { init(r, g, b, 1.); }
        Pixel(float r, float g, float b, float a) { init(r, g, b, a); }
        Pixel(float* P) { init(P[0], P[1], P[2]); }
        ...
        GLfloat& operator[](int i) { return Tpix[i]; }
...
class LUT
{
        // class enabling to convert an integer index in a unique RGB color
        // pixel,
...
test_LUT(int Nmax)
{
        // test que la classe LUT fonctionne bien ie bijection entre indice
        // entier et palette de couleur MC09
...
class Scene
{
        // classe beta a remplacer par la classe Canopy dans la radiosit�
        // mixte... construit juste une liste de triangles
      public:
        float Ttri[10][3][3];
        int N;
        Scene()
...
class GLscene
{
        ...
        // � partir d'une liste de couleur et utilisant la classe LUT pour
        // realiser la bijection etntre indice du triangle et couleur du Zbuffer
        // MC09
      private:
        Pixel Black;
...
// Truc pour passer une fction-membre � un gestionnaire de signal non C++ eg
// GLUT
...
idle()
{
        if (nrot < nrot_max) {
                ...
                Phi += M_PI / 180. * 5;
                printf(
                  "> idle: nrot=%d theta=%.1f\n", nrot++, Theta * 180 / M_PI);
                glutPostRedisplay();
        }
}

void
clavier(unsigned char touche, int x, int y)
{
        switch (touche) {
                case 'q':
                case 27:
...
void
GLscene::init(Scene& scene, int resX, int resY)
{
        // MC09
        Black.init(0, 0, 0, 1.);
        id = 1; // display list
        ...
        /* The X Window System specific options parsed by glutInit are as
           follows: -display DISPLAY Specify the X server to connect to. If not
           specified, the value of the DISPLAY environment variable is used.
           -geometry W x H + X + Y
           ...
           geometry specification. The effect of using this option is to change
           the GLUT initial size and initial position the same as if
        ...
        */
        ...
        if (resX > screenX) {
                ...
                resX = screenX;
                printf("<!> taille fenetre > taille ecran => resolution "
                       "recalc0ul�e... \n");
        } else if (resY > screenY) {
        ...
        printf(
          ">> resolution recalculee /ecran = resX=%d, resY=%d\n", resX, resY);
        ...
        // Version avec display List : declarer une fois la scene avant les
        // calculs de projection, +optimis�
        id = glGenLists(1);
        load(scene);
...
void
GLscene::project(double theta, double phi)
{
        ...
        */
        ...
        for (int x = 0; x < sw; x++)
                for (int y = 0; y < sh; y++) {
                        ...
                        reinterpret_cast<LPVOID>( &m_pix[0] ) ); glReadPixels(
                        0, 0, m_nWidth, m_nHeight, GL_RGB, GL_UNSIGNED_BYTE,
                        reinterpret_cast<GLvoid *>( &m_pix[0] ) ); ou encore
                        vector<GLubyte> store;
                        store.resize(Wide * Height * 4);
                        glReadPixels(0, 0, Wide, Height, GL_RGBA,
                        GL_UNSIGNED_BYTE, &store[0])
                        */
        ...
        //    printf(">>> Pixmap : width=%d, height=%d => pixel(%d, %d)=%.4f,
        //    %.4f,%.4f\n",  sw, sh, midx, midy, pixels[midx][midy][0],
        //    pixels[midx][midy][1], pixels[midx][midy][2] );
}

void
GLscene::load(Scene& scene)
{
        Pixel col;
        ...
        for (int i = 0; i < GLscene::N; i++) {
                ...
                glVertex3f(scene.Ttri[i][0][0],
                           scene.Ttri[i][0][1],
                           scene.Ttri[i][0][2]);
                glVertex3f(scene.Ttri[i][1][0],
                           scene.Ttri[i][1][1],
                           scene.Ttri[i][1][2]);
                glVertex3f(scene.Ttri[i][2][0],
                           scene.Ttri[i][2][1],
                           scene.Ttri[i][2][2]);
                glEnd();
        }
...
LUT::init(int Nval)
{
        // Construit la LUT entre un indice compris entre  0 et N et un triplet
        // de GLfloat RBV LUT=new GLfloat*[N];
        //  for (int i=0; i<N; i++) LUT[i]=new GLfloat[4];
        N = Nval;
        Tlut.resize(N);
        my_epsi = 1 / 300.;
        Npix =
          ceil(pow(N, 1 / 3.)); // Nb de pts au sein d'une composante couleur;
        dpix =
          255. /
          (Npix -
           1); // ecart de valeur entre 2 pts au sein d'une composante couleur
        ...
        for (int i = 0; i < N; i++) {
                ...
                if (nR >= Npix)
                        printf("<!> N=%d, i=%d: Npix=%d : Error depassement de "
                               "la LUT\n",
                               N,
                               i,
                               Npix);
...
void
LUT::idx2col(int i, Pixel& col)
{
        // Convertir un indice compris entre  0 et N en un triplet de GLfloat
        // RBV
        col[0] = Tlut[i][0];
        col[1] = Tlut[i][1];
        col[2] = Tlut[i][2];
        col[3] = 1;
        if (0)
                printf(">> idx2col()i=%d,  col=(%.2f, %.2f, %.2f )\n",
                       i,
                       col[0],
                       col[1],
                       col[2]);
...
LUT::col2idx(Pixel& col)
{
        ...
        for (int i = 0; i < 3; i++) {
                ...
                if (0)
                        printf(">>> i=%d: ipix=%.f, icol=%.2f, (int)=%d\n",
                               i,
                               ipix[i],
                               (255 - ipix[i]) / (double)dpix,
                               icol[i]);
        }
        idx = icol[0] * Npix * Npix + icol[1] * Npix + icol[2];
        if (0)
                printf("> col2idx(): iR=%d, iG=%d, iB=%d => idx=%d => ",
                       icol[0],
                       icol[1],
                       icol[2],
                       idx);
        ...
        Pixel pix;
        idx2col(idx, pix);
        if ((fabs(pix[0] - col[0]) < my_epsi) &&
            (fabs(pix[1] - col[1]) < my_epsi) &&
            (fabs(pix[2] - col[2]) <
             my_epsi)) // Ne marche pas ??? => Ok pb d'epsi var globale quelque
                       // part :-( => chger en my_epsix
                //  if( pix[0]==col[0] && pix[1]==col[1] && pix[2]==col[2]
                //  )//temporaire ,mais marche !
                return idx;
        else {
...
/* Notes - MC09
   ...
   1) Memes resultats (en terme de nb de pixels colore's)en onscreen (avec
   glutMainLoop) et en offscreen (isglut=0) 2) resolution ne peut pas exceder
   670 x 670 sur le Mac !!!! => grosse limitation
