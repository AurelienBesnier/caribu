...
msailad(Msailin& msailin)
{
        ...
        for (ic = 1; ic < N; ic++) { // boucle globale sur les couches
                ...
                psir = rd * msailin.psi;
                // Appel de la fonction de distribution des angles foliaire,
                // fonction Beta
                if (msailin.nbang == 13) {
                        u = msailin.bmu[ic];
                        v = msailin.bnu[ic];
                        ...
                        if (ic == 1)
                                printf(
                                  "==> Distribution Beta d angle folaire\n");
                }
                // Initialisations
                ...
                for (ia = 0; ia < msailin.nbang; ia++) {
                        if (msailin.nbang == 13) {
                                // 13 classe d'angles de Verhoef avec
                                // distribution Beta
                                ...
                                // Cas de classes d'angles foliaires mesure'es
                                // par s4

                                // msailin.f(ic,ia) *= 100.;//deplacer dans le
                                // main a cuase des boucle de direction!
                                ttl =
                                  90. / (REEL)msailin.nbang * ((REEL)ia + .5);
                        }
                        fcum += msailin.f(ic, ia);
                        // printf("tm=%4.1lf - f(%d)=%6.3lf -
                        ...
                        /*
                          ...
                          bts et bto sont les angles de transition beta s et
                          beta o pour les directions d'incidence et de visee.
                          bts peut etre calcule lorsque ttl+tts > pi/2
                          alors si bts < azimut feuille < 2pi - bts , les
                          feuilles sont eclairees par leur face inferieure. Pour
                          tts+ttl < pi/2 les feuilles sont toujours eclairees
                          par leur face superieure; bts = pi pour assurer la
                          continuite des formules Rq: bto `a une signification
                          equivalente dans la direction de visee
                          */
                        ...
                        // Definition des angles azimutaux auxiliaire bt1, bt2,
                        // bt3 utilises dans le calcul du coefficient de
                        // diffusion bidirectionnelle w
                        btran1 = fabs(bts - bto);
                        btran2 = pi * 2 - bts - bto;
                        if (psir <= btran1) {
                                bt1 = psir;
                        ...
                        sks +=
                          ((bts - pi * .5) * cstl + sin(bts) * tants * sntl) *
                          msailin.f(ic, ia);
                        sko +=
                          ((bto - pi * .5) * cstl + sin(bto) * tanto * sntl) *
                          msailin.f(ic, ia);
                        ...
                        if (bt2 != 0.)
                                sw2 +=
                                  (-bt2 * t1 +
                                   sin(bt2) * (cs2tl / (cos(bts) * cos(bto)) +
                                               cos(bt1) * cos(bt3) * tsin)) *
                                  msailin.f(ic, ia);
                } // for ia
                /**********************************************************************
                 *                  COEFFICIENTS DU SYSTEME DE SUITS *
                 *                                                                    *
                 *    ATT  coefficient d'attennuation du flux diffus * SIG
                 *coefficient de retrodiffusion du flux diffus               *
                 *    KS   coefficient d'extinction du flux direct incident * KO
                 *coefficient d'extinction dans la direction de visee        *
                 *    SF   coefficient de diffusion du flux direct incident en *
                 *         flux diffus  descendant * SB   coefficient de
                 *diffusion du flux direct incident en        * flux diffus
                 *montant (retrodiffusion)                       * UF
                 *coefficient de diffusion dans la direction de visee        *
                 *         du flux diffus montant * UB   coefficient de
                 *diffusion dans la direction de visee        * du flux diffus
                 *descendant (retrodiffusion)                 * W    coefficient
                 *de diffusion bidirectionnelle                  *
                 *                                                                    *
                 **********************************************************************/
                ...
                printf(
                  "<!> sks=%.3g\t msailin.l[%d]=%.3g\t => playin->ks = %.3g\n",
                  sks,
                  ic,
                  msailin.l[ic],
                  playin->ks);
                ...
                playin->w = msailin.l[ic] * .01 *
                            (msailin.roo[ic] * .01 * sw1 + 2 * rtp * sw2 / pi);
        } // for ic couche
 ...
 *                                                                              *
 *   Resolution du systeme d'equations differentielles decrivant les * les
 *transferts radiatifs dans un couvert de SAIL                           *
 *   VERHOEF 1985 *
 *                                                                              *
 *******************************************************************************/

void
mlayer(Msailin& msailin, Mlayout* Tlayout)
{
        ...
        if (Tlayin == NULL) {
                fprintf(
                  stderr,
                  "<!> mlayer() appelee avant msailad() : Tlayin pas alloue\n");
                exit(-1);
        }
        layout = Tlayout;
        // printf("mlayer(): N=%d\n",N);
        for (ic = 0; ic < N; layout++, ic++) {
                if (msailin.l[ic] == 0) {
                        ...
                        cs = (layin.sb * (layin.ks - layin.att) -
                              layin.sf * layin.sig) /
                             dns;
                        ds = (-layin.sf * (layin.ks + layin.att) -
                              layin.sb * layin.sig) /
                             dns;
                        co = (layin.uf * (layin.ko - layin.att) -
                              layin.ub * layin.sig) /
                             dno;
                        do_ = (-layin.ub * (layin.ko + layin.att) -
                               layin.uf * layin.sig) /
                              dno;
                        // Coefficients permettant le calcul des differents flux
                        // sortants `a partir des flux entrants :
                        ...
                        layout->rsd = cs * (1 - layout->tss * layout->tdd) -
                                      ds * layout->rdd;
                        layout->tsd = ds * (layout->tss - layout->tdd) -
                                      cs * layout->tss * layout->rdd;
                        layout->rdo = co * (1 - layout->too * layout->tdd) -
                                      do_ * layout->rdd;
                        layout->tdo = do_ * (layout->too - layout->tdd) -
                                      co * layout->too * layout->rdd;

                        ho = (layin.sf * co + layin.sb * do_ + layin.w) /
                             (layin.ko + layin.ks);
                        layout->rso = ho * (1 - layout->tss * layout->too) -
                                      co * layout->tsd * layout->too -
                                      do_ * layout->rsd;
                } // else
        }         // for ic
        delete[] Tlayin;
...
/****************************************************************************
   ...
   *************************************************************************/
void
distrib(double* fbeta, double u, double v)
{
