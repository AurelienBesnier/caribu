...
main(int argc, char** argv)
{
        ...
        // mise en place d'une back-door ;-)  pour calculer la BRDF du couvert
        // ss hot spot par mcsail
        //  debut MC09
        ifstream fvisee;
        ofstream fbrf;
        ...
        if (fvisee.is_open()) {
                brdf = true;
                fvisee >> N_visee;
                cerr << "<!> Presence d'un fichier VISEE => activation du mode "
                        "canopy BRDF - MC09\n Nb de visees = "
                     << N_visee << endl;
                fbrf.open("sail_brf.dat", ios::out);
        }

        for (int iv = 0; iv < N_visee;
             iv++) { // boucle pour la BRDF du couvert - MC09
                if (brdf) {
                ...
                for (i = 0; i < N + 1; i++) {
                        Cprofout[i].transdir = Cprofout[i].transdif =
                          Cprofout[i].trans = 0;
                        Cprofout[i].refdif = Cprofout[i].refdir =
                          Cprofout[i].absc = 0;
                }
                // => LEAFAREA
                ...
                for (i = N - 1; i > 0; i--) {
                        ...
                        fpar2 >> msailin.bmu[i] >> msailin.bnu[i] >>
                          msailin.l[i];
                        sf += msailin.l[i];
                ...
                if (npo != N - 1) {
                        fprintf(stderr,
                                "<!>\tNumber of optical properties layers <> N "
                                "number of layers\n"
                                "\t only the first line of spectral taken into "
                                "acount %c\n",
                                7);
                        npo = 1;
                }
                ...
                fpar2.close();
                // BUG MCFeb2006: cas ou il n' y a pas de sol dans la maquette
                // =raz de ros
                if (argc > 2)
                        ros = 0.;
                ...
                if (!fpar2.is_open()) {
                        fprintf(stderr,
                                " File %s does'nt exist => %s aborted...%c\n",
                                skyname,
                                argv[0],
                                7);
                        return -1;
                ...
                for (js = 0; js < nbs; js++) {
                        if (riri) { // Version compatible A. Olioso et Riri =>
                                    // INCIDENT
                                Esource = 1;
                                ...
                                msailin.tts =
                                  acos(fabs(dir[2]) /
                                       sqrt(dir[0] * dir[0] + dir[1] * dir[1] +
                                            dir[2] * dir[2])) /
                                  rd;
                                printf("    Esource=%.2g, theta_source=%.2g "
                                       "[nbs=%d]\n",
                                       Esource,
                                       msailin.tts,
                                       nbs);
                        ...
                        limit.es = 1 - limit.ed;
                        // appel aux sous programmes de calcul des coefficients
                        // de Verhoef
                        for (i = 0; i < N + 1; i++) {
                                Tprofout[i].transdir = Tprofout[i].transdif =
                                  Tprofout[i].trans = 0;
                                Tprofout[i].refdif = Tprofout[i].refdir =
                                  Tprofout[i].absc = 0;
                        }
                        // msail.C
                        msailad(msailin);
                        ...
                        // Ferr<< "mlayer() : Fin"<<'\n';
                        //   Matrices de reflectance et de transmittance  de
                        //   chaque couche
                        // Ferr<<"mprofil() : Debut"<<'\n';
                        ...
                        for (i = 0; i < N + 1; i++) {
                                // printf("Tprofout[%d].transdir=%.3g -
                                // ",i,Tprofout[i].transdir);
                                Cprofout[i].transdir +=
                                  Esource * Tprofout[i].transdir;
                                Cprofout[i].transdif +=
                                  Esource * Tprofout[i].transdif;
                                Cprofout[i].trans +=
                                  Esource * Tprofout[i].trans;
                                Cprofout[i].refdif +=
                                  Esource * Tprofout[i].refdif;
                                Cprofout[i].refdir +=
                                  Esource * Tprofout[i].refdir;
                ...
                if (!brdf) {
                        ...
                        fprintf(fenv, "%d  %lf\n", N - 1, dz);
                        printf("ic  alti  clai   l(ic)  pene   Fdown  Fup  Rf  "
                               " Tf\n");
                        fflush(stdout);
                        for (i = 1; i < N + 1; i++) {
                                profout = Cprofout[i];
                                clai += msailin.l[i - 1];
                                printf("%2d  %5.3f  %5.3f  %5.3f  %5.3f  %5.3f "
                                       " %5.3f  %5.3f  %5.3f\n",
                                       i + 1,
                                       (i - 1) * dz,
                                       sf - clai,
                        ...
                        if (phi_v == 0)
                                theta_v = -theta_v;
                        printf(
                          "BRDF: tv=%.1lf, phi=%.1lf %.3lf\n",
                          theta_v,
                          phi_v,
                          Cprofout[N - 1].refdir); //,  Cprofout[N-1].refdif);
                        fbrf << theta_v << "\t" << Cprofout[N - 1].refdir
                             << "\t" << Cprofout[N - 1].refdif << endl;
                } // else if ! brdf
