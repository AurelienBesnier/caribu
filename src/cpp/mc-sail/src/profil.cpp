...
/*******************************************************************************
 *             Albert OLIOSO * Mars 1994 * Calcul des matrices de reflectance et
 *de transmittance           * et des parametres radiatifs d'un couvert
 *multicouche             *
 *                                                                              *
 *******************************************************************************/
struct Coeff
{
        REEL RMdd, TMdd, RMdo, RMsd, TMsd, TMds, RMso, TMss, TMTdd, TMTsd,
          TMTds, TMTss, den;
};

void
mprofil(Limit& limit, Profout* Tprof, Mlayout* Tlay)
{
        int ic;
        Coeff *T, *o;
        Mlayout l;
        ...
        for (ic = 1; ic < N + 1; ic++, o++) {
                ...
                o->RMsd =
                  l.rsd + (l.tss * T[ic - 1].RMsd + l.tsd * T[ic - 1].RMdd) *
                            l.tdd / o->den;
                o->RMdo =
                  l.rdo + l.tdd *
                            (T[ic - 1].RMdd * l.tdo + T[ic - 1].RMdo * l.too) /
                            o->den;
                o->RMso =
                  (l.tss * T[ic - 1].RMsd + l.tsd * T[ic - 1].RMdd) * l.tdo;
                o->RMso = o->RMso + (l.tsd + l.tss * T[ic - 1].RMsd * l.rdd) *
                                      T[ic - 1].RMdo * l.too;
                o->RMso =
                  l.rso + l.tss * T[ic - 1].RMso * l.too + o->RMso / o->den;
                o->TMss = l.tss;
        ...
        for (ic = N - 1; ic > 0; ic--) {
                ...
                T[ic].TMTsd =
                  T[ic].TMsd * T[ic + 1].TMTss + T[ic].TMdd * T[ic + 1].TMTsd;
                T[ic].TMTds = T[ic].TMss * T[ic + 1].TMTds;
                T[ic].TMTdd =
                  T[ic].TMdd * T[ic + 1].TMTdd + T[ic].TMsd * T[ic + 1].TMTds;
        } // for ic
        ...
        for (ic = 1; ic < N + 1; ic++) {
                ...
                Tprof[ic].transdif =
                  (T[ic].TMTsd * limit.es + T[ic].TMTdd * limit.ed);
        ...
        for (ic = N; ic > 0; ic--) {
                ...
                M(i, i) = M(i + 1, i + 1) = M(i + 2, i + 2) = M(i + 3, i + 3) =
                  1;
