...
class DiffReel : public Diffuseur
{
      protected:
        double Eabs[2], E1a[2],
          E2a[2]; // 2 valeurs=ordre 0 et ordres>1 de rediffusion
        Primitive* prim;
        Actop* opti;
        long vis_max;
        long vis_cur;
      ...
      public:
        ...
        {
                prim->show();
                cout << "curvis = " << vis_cur << " - maxvis = " << vis_max
                     << endl;
        }
        // void fini() {opti->fini();}
};
...
class DiffO : public DiffReel
{
      ...
      public:
        ...
        {
                delete prim;
        } // On ne delete pas *opti car il est partage avec d'autres diff
        DiffO(Primitive* p, Actop* actop)
          : DiffReel(p, actop)
        {
...
class DiffT : public DiffReel
{
      private:
        ...
        Actop* popt(Face side) { return (side == sup) ? opti : optback; }

      public:
        ~DiffT() { delete prim; }
        DiffT(Primitive* p, Actop* actop, Actop* backopt)
          : DiffReel(p, actop)
        {
                ...
                nk[0] = nk[1] = 0.0;
                // printf("DiffT::cstructor (%.0lf):: avant
                // actop->koi()",p->name());
                // opti->koi();
                optback = backopt;
                // printf("DiffT::cstructor (%.0lf):: avant optback
                // ->koi()",p->name()); optback->koi();
        }
        // virtual  Param_Inter interact(Param_Inter&,bool,int )=0;
...
class Diff8 : public Diffuseur
{
      ...
      public:
        ~Diff8() { delete prim; }
        Diff8() { cerr << "Diff8 [constructeur] Warning : no parameters!\n"; }
        ...
        double lux(Param_Inter& parag) { return diff->lux(parag); }
        bool isopaque() { return diff->isopaque(); }
        ...
        void norm_surf(int nb_ray, double Stoit)
        {
                nb_ray = 0;
                Stoit = 0;
        }

        double nrj(int ordre) { return diff->nrj(ordre); }
        double VEa(int ordre) { return diff->VEa(ordre); }
        double rad(char face) { return diff->rad(face); }
        double inc(char face) { return diff->inc(face); }
        double ncontact(char face = 0) { return diff->ncontact(face); }
        double Vrad(char face) { return diff->Vrad(face); }
        double rad1(char face) { return diff->rad1(face); }
        double Vrad1(char face) { return diff->Vrad1(face); }
        Visiblite is_visible()
        {
                if (vis_cur == 0)
                        return pasvu;
...
class DiffP : public Diffuseur
{
      ...
      public:
        ...
        Tabdyn<double, 4>
          fluxm; //[2],inf/sup, [6] ordres, [4] 1/2plans,[18] teta;
        Tabdyn<double, 2>
          phim;      //[2],inf/sup, [6] ordres : valeur du flux hemispherique
        double pene; //[nb] couches : penetration du direct
        // fonctions-membres
        DiffP() { cout << "DiffP [constructeur] Warning : no parameters!\n"; }
        DiffP(Primitive* pprim);
        ~DiffP();
        Primitive& primi() { return *prim; }
        // int nb_som(){ return prim->nb_sommet(); }
        bool isreal() { return true; }
        bool actif() { return false; }
        Param_Inter interact(Param_Inter&, bool, int);
        double lux(Param_Inter&) { return 0.; }
        bool isopaque() { return false; }
        void maj_stat(int) {}
        void calc_stat(int i, double Stoit)
        {
                i = 0;
                Stoit = 0;
        }
        virtual void norm_surf(int i, double x)
        {
                i = 0;
                x = 0;
        }
        ...
        Visiblite is_visible() { return pasvu; }
        DiffP& operator++(int) { return *this; }
        DiffP& operator--(int) { return *this; }
        double intersect(Param_Inter parag, Point* I)
        {
                return (prim->intersect(parag, I));
...
class DiffP2 : public Diffuseur
{
      ...
      public:
        ...
        int No;
        // fonctions-membres
        DiffP2() { cout << "DiffP2 [constructeur] Warning : no parameters!\n"; }
        ...
        Visiblite is_visible() { return pasvu; }
        DiffP2& operator++(int) { return *this; }
        DiffP2& operator--(int) { return *this; }
        double intersect(Param_Inter parag, Point* I)
        {
                return (prim->intersect(parag, I));
