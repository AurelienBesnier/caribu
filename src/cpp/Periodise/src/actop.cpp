...
//-*************** lambert() **********************
inline void
Lambert::lambert(double& teta, double& phi)
{
...
//-*************** fresnel() ***********************
inline double
Fresnel::fresnel(double& teta, double& phi)
{
        double Fr;
        static bool fst = true;
        phi = phi;

        // Formule de Fresnel, par approx. Schlick (fraction rationnelle) error
        // <1%
        ...
        if (hair > 0.0) {
                ...
                // if(Fr>1) printf(" Fr_a >1 - Fr_a= %.4g - teta =
                // %.4g\n",Fr,teta/rad);
        }
        if (fst) {
                printf(
...
//-*************** lobeG() ***********************
inline double
Gauss::lobeG(double& teta, double& phi)
{
        double Fr, um, g, Z, spread;
        static bool fst = true;
        double t = teta, p = phi, cost, sint, sing,
               sc; // necessaire a cause de la pertub angulaire de Fresnel
        int iter = 0;
        ...
        do {
                ...
                // if(rand()>0.5) g=-g; // pour avoir les 2 brins de la
                // gaussienne teta_r
                ...
                if (iter == 10)
                        fprintf(
                          stderr,
                          "<10> Gauss:lobeG() boucle de calcul de g: iter= 10 "
                          "=> teta=%.1g, zeat2=%.6g ==> spread=%.6g\n",
                          teta / rad,
                          zeta2,
                          spread);
                if (iter == 50)
                        fprintf(
                          stderr,
                          "<50> Gauss:lobeG() boucle de calcul de g: iter= 50 "
                          "=> teta=%.1g, zeat2=%.6g ==> spread=%.6g\n",
                          teta / rad,
                          zeta2,
                          spread);
                if (iter == 100)
                        fprintf(
                          stderr,
                          "<100> Gauss:lobeG() boucle de calcul de g: iter= "
                          "100 => teta=%.1g, zeat2=%.6g ==> spread=%.6g\n",
                          teta / rad,
                          zeta2,
                          spread);
                if (iter == 500)
                        fprintf(
                          stderr,
                          "<500> Gauss:lobeG() boucle de calcul de g: iter= "
                          "500 => teta=%.1g, zeat2=%.6g ==> spread=%.6g\n",
                          teta / rad,
                          zeta2,
                          spread);
...
//-*************** Lambert::lux() ****************************
double
Lambert::lux(Transf& param)
{
        if (param.tetaeye >= 0.0)
                return reflectance / M_PI;
        else
                // cas opaque avec transmit=0 traite au niveau diffuseur (pas de
                // pb de /0.0)
...
//-*************** Fresnel::lux() ****************************
double
Fresnel::lux(Transf& param)
{
        // faux : fait pour pas avoir de plantage si l'image est nulle
        return fresnel(param.teta, param.phi);
        // rajouter l'angle solide de l'objectif pour la proba d'interception du
        // spec
        /*  if ((fabs(param.tetaeye-param.teta)<1e-6)&&
          (fabs(fabs(param.phieye-param.phi)-M_PI)<1e-6)) return
          fresnel(param.teta, param.phi); else return 0; //pas dans la dir refl.
          speculaire
        */
} // Fresnel::lux()
...
//-*************** Specdifu::interact() ***********************
void
Specdifu::interact(Transf& param)
{
        ...
        /*  if(Rspec+Rdiff>1.0){
            cerr<<"\n\n\t#*> Violation de la loi de conservation de l'E : Rspec
           +Rdiff="<<Rspec+Rdiff<<endl; fprintf(stderr,"Rf= %.4g - Tf= %.4g -
           Rspec= %.4g \n",reflectance,transmit, Rspec); exit(-1);
            }
        */
        // printf("Specdifu[interact] Rspec=%g, verif conservation . E =
        // %g\n",Rspec,Rspec+Rdiff);
        if (rand.tirage() < Rdiff / (Rspec + Rdiff)) {
                ...
                if (transmit > 0.0) {
                        double tir = rand.tirage();
                        // cout <<" Specdifu[interact] tir = "<<tir<<" - ref =
                        // "<<reflectance<<endl;
                        if (tir >= reflectance / (transmit + reflectance)) {
                                param.teta = M_PI - param.teta;
                                // cout<<"\t\t **** TRANSMISSION ****\n";
        ...
        param.rediffuse *=
          Rspec + (1.0 - Rspec) * reflectance; // a boucler pae eq. bilan d'E
        // Einc=Eabs+Erefl+Etrans=Eabs+(Espec+ErefD)+Etrans
} // Specdifu::interact()

//-*************** Specdifu::lux() ****************************
double
Specdifu::lux(Transf& param)
{
...
Gauss::lux(Transf& param)
{
        ...
        /*  if ((fabs(param.tetaeye-param.teta)<1e-6)&&
          (fabs(fabs(param.phieye-param.phi)-M_PI)<1e-6)) return
          Gauss(param.teta, param.phi); else return 0; //pas dans la dir refl.
          speculaire
        */
} // Gauss::lux()
...
Ross::interact(Transf& param)
{
        ...
        */
        ...
        if (rand.tirage() < Rdiff / (Rspec + Rdiff)) {
                ...
                if (transmit > 0.0) {
                        double tir = rand.tirage();
                        // cout <<" Ross[interact] tir = "<<tir<<" - ref =
                        // "<<reflectance<<endl;
...
RossL::interact(Transf& param)
{
        ...
        // distribue angulairement la reflectance et la transmission de facon
        // lambertienne
        lambert(param.teta, param.phi);
        if (transmit > 0.0) {
                double tir = rand.tirage();
                // cout <<" RossL[interact] tir = "<<tir<<" - ref =
                // "<<reflectance<<endl;
                if (tir >= R / (R + T)) {
                        param.teta = M_PI - param.teta;
                        // cout<<"\t\t **** TRANSMISSION ****\n";
...
/******************************************/
double
Sol::refbd(double ts, double to, double psi)
{
...
Sol::refdh(double ts)
{
        ...
        r3 = (ci * 2. * (g + 1.) + 1.) / (gg);
        tmp = 3 * Pci[1] - 1;
        r4 = ci * (4 * rb * Pci[1] - rc * tmp * tmp - 4) * log((ci + 1.) / ci) /
             4.;                            /* /4 ou /4. ?*/
        r5 = rb * ci * (ci * 2. - 1.) / 2.; /* /2 ou /2. ?*/
        r6 = 0.375 * rc * ci * (6. * Pci[2] - 3 * Pci[1] - 2 + 1);

        return (r1 - r2 - r3) * w * (g - 1.) / (gg * ci + 1.) +
               (r4 - r5 + r6 + 1.) * w / 2.;
}
...
//-*************** Sol::interact() ***********************
void
Sol::interact(Transf& param)
{
        // 1st sol : Comme Ross sauf que je reaxe dans le hot spot par un
        // phi-=M_PI
        //  Gauusien + lambert, mais a partir de valeur de SOILSPEC et un calage
        //  a la mano
        ...
        if (Rdh > 1.0) {
                cerr << "\n\n\t#*> Violation de la loi de conservation de l'E "
                        ": Rdh="
                     << Rdh << endl;
                exit(-1);
        }
        ...
        if (rand.tirage() < Rdiff / Rbd) {
                ...
                do {
                        ...
                        if (fabs(param.teta) > 0.0001) {
                                p =
                                  Macos((-cos(g) * sint - sc * cost) / sin(t));
                                sinp = sin(p);
                                if (fabs(sinp) < 0.0001)
                                        p += M_PI;
                                else if (sinp >
                                         0) // car Macos -> [0:pi] et sin(g)>0
                                        p = M_PI * 2. - p;
                        }
                } while (t > M_2PI);
                param.teta = t;
