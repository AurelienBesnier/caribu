...
DiffO::lux(Param_Inter& parag)
{
        ...
        return opti->lux(parao) * parag.poid() *
               fabs(cos(M_PI_2 - parao.tetaeye));
...
DiffT::lux(Param_Inter& parag)
{
        ...
        return popt(side)->lux(parao) * parag.poid() *
               fabs(cos(M_PI_2 - parao.tetaeye));
} // DiffT::lux()
...
Param_Inter
DiffO::interact(Param_Inter& parag, bool inside, int order)
{
        Transf parao;
        Vecteur rediff;
        ...
        if (scal > 0.0) {
                //  printf(" Si pas infini, bug... Opaque attaque' par
                //  de'rrie`re");
                // prim->qui();
                // cout<<" Normale  : ";prim->normal().show();
                // cout<<" Incident : ";parag.direct().show();
        ...
        opti->interact(parao);
        //   cout <<"DiffO[interact] teta redif = "<<parao.teta<<" - phi =
        //   "<<parao.phi<<endl;
        rediff = parag.direct();
        ...
        */
        ...
        // if(parao.rediffuse>0.5) cout<<"interact poid rediffuse =
        // "<<parao.rediffuse<<endl;
        ...
        // cout<<"DiffO[interact()] abs="<<parao.abs<<"=> Eabs ="<<Eabs<<" -
        // Poids = "<<parag.poid()<<endl; cout <<" DiffO[interact()] new direct
        // ";parag.direct().show();
        return parag;
}
...
Param_Inter
DiffT::interact(Param_Inter& parag, bool inside, int order)
{
        Transf parao;
        ...
        inside = inside;
        // cout<<"DiffT[interact()]poid = "<<parag.poid()<<" Face
        // :"<<side;prim->qui();
        ...
        // cout <<"DiffT[interact] scal = "<<scal<<" normale =
        // ";N.show();fflush(stdout);
        parao.phi = 0.0;
        // appel a Actop[side]
        ...
        \n",(int)side);fflush(stdout); popt(side)->koi(); printf(" Apres
        Actop::koi() \n");fflush(stdout);
        ...
        */
        ...
        // cout <<"DiffT[interact] teta redif =
        // "<<parao.teta<<endl;fflush(stdout);
        ...
        // if(radio[side]<10) {cout<<"DiffT[interact()]  radio
        // ="<<radio[side]<<endl;show();}
        ...
        if (order == 0) {
                ...
                Eabs[1] += parao.abs;
        // if(radio[side]<10) cout<<"                   radio
        // ="<<radio[side]<<endl;
        radio[side] += Etrans;
        nk[side] += 1; // cout<<"nk+=1\n";
        parag.change_poids(Ei);

        // cout<<"DiffT[interact()] abs="<<parao.abs<<"=> Eabs ="<<Eabs<<" -
        // Poids = "<<parag.poid()<<endl;
        // parag.direct().show();
        return parag;
} // DiffT::interact()
...
void
DiffT::calc_stat(int niter, double Stoit)
{
        raus((niter < 1),
             "Diffuseur[calc_stat] appel sans specifier le nb d'iterations!");
        ...
        E2a[0] =
          sqrt((E2a[0] / (n / Stoit / Stoit) - (E1a[0] * E1a[0])) / (n - 1.5));
        E2a[1] =
          sqrt((E2a[1] / (n / Stoit / Stoit) - (E1a[1] * E1a[1])) / (n - 1.5));
        // Incident
        Einc[0] = E1i[0] /= n * surf;

        E2i[0] =
          sqrt((E2i[0] / (n * surf * surf) - (E1i[0] * E1i[0])) / (n - 1.5));
        Einc[1] = E1i[1] /= n * surf;
        E2i[1] =
          sqrt((E2i[1] / (n * surf * surf) - (E1i[1] * E1i[1])) / (n - 1.5));
        // radiosite
        radio[0] = radio1[0] /= n * surf;
        radio2[0] =
          sqrt((radio2[0] / (n * surf * surf) - (radio1[0] * radio1[0])) /
               (n - 1.5));
        radio[1] = radio1[1] /= n * surf;
        radio2[1] =
          sqrt((radio2[1] / (n * surf * surf) - (radio1[1] * radio1[1])) /
               (n - 1.5));
        // Radiosite 1er ordre
        B1st[0] = B1st1[0] /= n * surf;
        B1st2[0] = sqrt((B1st2[0] / (n * surf * surf) - (B1st1[0] * B1st1[0])) /
                        (n - 1.5));
        B1st[1] = B1st1[1] /= n * surf;
        B1st2[1] = sqrt((B1st2[1] / (n * surf * surf) - (B1st1[1] * B1st1[1])) /
                        (n - 1.5));
...
void
DiffO::calc_stat(int niter, double Stoit)
{
        raus((niter < 1),
             "Diffuseur[calc_stat] appel sans specifier le nb d'iterations!");
        // calcul de la moyenne et de la variance
        ...
        radio = radio1 /= n * surf;
        radio2 =
          sqrt((radio2 / (n * surf * surf) - (radio1 * radio1)) / (n - 1.5));
...
void
DiffT::norm_surf(int nbr, double Stoit)
{
...
void
DiffO::norm_surf(int nbr, double Stoit)
{
        // calcul de la moyenne et de la variance
        // calcul de la moyenne et de la variance
        double surf;
...
Param_Inter
DiffP::interact(Param_Inter& parag, bool inside, int ordres)
{
        ...
        // printf("; deltaZ=%g,Macos(z)=%g,theta=%.2lf,
        // tetad=%d\n",fabs(delta[2]), Macos(fabs(delta[2])),theta ,tetad);
        // fflush(stdout);
        jj = (delta[2] > 0) ? 0 : 1;
        if (theta != 0.0) {
                phi = Macos(delta[0] /
        ...
        if (order == -1) {
                ...
                CO(printf("DiffP[interact] : pene(%5.2f)= %g\n", Z, pene);
                   fflush(stdout);)
                //  printf("DiffP[interact] : jj=%d,order=%d,phid=%d,tetad=%d ::
                //  %.0g\n",(int)jj,(int)
                //  order,(int)phid,(int)tetad,prim->name());fflush(stdout);
                // printf("%d,",Z);
        } else {
                phim(jj, order) += weight;
                if (phid < 4) { // remplissage matrice flux directionnel
                        // printf("DiffP[interact] :
                        // jj=%d,order=%d,phid=%d,tetad=%d ::
                        // %.0g\n",(int)jj,(int)
                        // order,(int)phid,(int)tetad,prim->name());fflush(stdout);
...
Param_Inter
DiffP2::interact(Param_Inter& parag, bool inside, int ordres)
{
        double weight, scal;
