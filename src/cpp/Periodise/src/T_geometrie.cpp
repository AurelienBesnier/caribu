...
double
Vecteur::prod_scalaire(const Vecteur& v1)
{
        return (v1.homo[0] * homo[0] + v1.homo[1] * homo[1] +
                v1.homo[2] * homo[2]);
}

Vecteur
Vecteur::prod_vectoriel(const Vecteur& v1) const
{
        Vecteur res;
...
*/
double
Vecteur::prod_mixte(const Vecteur& v1, const Vecteur& v2) const
{
        double res;
        Vecteur u = v2;
        Vecteur inter = u.prod_vectoriel(v1);
...
{
        return (
          sqrt(homo[0] * homo[0] + homo[1] * homo[1] + homo[2] * homo[2]));
}

const Vecteur
Vecteur::normalisation()
{
        double norm;
        Vecteur res;
...
void
Vecteur::formation_vecteur(Point& p1, Point& p2)
{
...
Vecteur::rotate(Vecteur& axe, double teta)
{
        ...
        ax2 = axe[1] * sina;
        ax3 = axe[2] * sina;
        res[0] = (ax11 + cosa) * homo[0] + (ax12 - ax3) * homo[1] +
                 (ax13 + ax2) * homo[2];
        res[1] = (ax12 + ax3) * homo[0] + (ax22 + cosa) * homo[1] +
                 (ax23 - ax1) * homo[2];
        res[2] = (ax13 - ax2) * homo[0] + (ax23 + ax1) * homo[1] +
                 (ax33 + cosa) * homo[2];
        return res;
} // rotate()
...
Point
Point::operator+(const Homogene& vect)
{
        Point res;

        if (vect.homo[3] == 1) {
                cout << "ERREUR - Impossible d'additionner deux points";
                cout.flush();
                exit(1);
        } else {
...
Point::operator-(const Homogene& vect)
{
        ...
        if (vect.homo[3] == 1) {
                cout << "ERREUR - Impossible de soustraire deux points";
                cout.flush();
                exit(1);
        } else {
...
Point&
Point::operator+=(const Vecteur& vect)
{ // Translation de vect :MC
        homo[0] += vect.homo[0];
        homo[1] += vect.homo[1];
        homo[2] += vect.homo[2];
...
reel
Point::distance_pt_plan(Point& a, Point& b, Point& c)
{
        Vecteur AB, AC, AP;
        Vecteur ABN, ACN;
        reel res;
