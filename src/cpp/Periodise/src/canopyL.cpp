...
void
Canopy::gencan(char* fname)
{
        // genere a partir de la liste le fichier can avec indice neg pour les
        // diff 8 et
        ...
        for (liste_diff_scene.debut(); !liste_diff_scene.finito();
             liste_diff_scene.suivant()) {
                ...
                if (diff->actif()) {
                        ...
                        fprintf(fic,
                                "p 2 %.0f %d   %d  ",
                                (double)prim->name() * 1000,
                                label,
                                nbs);
                        for (i = 0; i < nbs; i++) {
                                fprintf(fic,
                                        " %f %f %f ",
                                        (double)prim->sommets(i)[0],
                                        (double)prim->sommets(i)[1],
                                        (double)prim->sommets(i)[2]);
                        }
                        fprintf(fic, "\n");
                        nbd++;
                } else
...
// FORMAT FICHIER : 1entier : libelle ; 3 coordonees (double) pour 3 sommets
// d'un triangle
...
lectop(ifstream& fopti, bool opac = false)
{
        ...
        switch (c) {
                ...
                case 's': // specular (Fresnel's law + attenation(van der Bild)
                        if (!opac) {
                                cerr << "speculaire pas prevu today pour "
                                        "non-opaques!\n";
                                exit(-1);
                        }
                ...
                case 'm':                 // mixte (Lambert + specular)
                        ...
                        actop =
                          new Specdifu(popt[0], popt[1], popt[2], popt[3]);
                        break;
                case 'g': // speculaire a lobe Gaussien
                        if (!opac) {
                                cerr << "speculaire gaussien pas prevu today "
                                        "pour non-opaques!\n";
                                exit(-1);
                        }
                        fopti >> popt[0]; // indice de refraction n
                ...
                case 'r': // Ross model (Lambert + Gaussian specular)
                        ...
                        actop =
                          new Ross(popt[0], popt[1], popt[2], popt[3], popt[4]);
                        break;
                case 'S': // SOIL model (Lambert + Gaussian hot spot
                          // (~SOILSPEC))
                ...
                case 'n': // neural network's regression
                        cerr << "@!$ description des prop. opt. par reseau de "
                                "neurones pas implementee\n";
                        exit(-1);
                        break;
                default:
...
endline(ifstream& fin)
{
        ...
        linie[k] = 0;
        // cerr<<"\tCanopy[parse_can] finligne : chaine = "<<linie<<"de longueur
        // "<<k<<endl;
        return linie;
} // endline()
                  ...
                  char* name8)
{
        ...
        if (!nopti_is_null && file_exists) {
                ...
                if (nopti_is_null) {
                        cerr << "Attention - Pas de proprietes optiques"
                             << endl;
        ...
        if (name8 == NULL)
                ...
                fdim.close();
                for (i = 0; i < 3; i++)
                        cout << "Infty: Bornemax[" << i << "] = " << bornemax[i]
                             << "  Bornemin[" << i << "] = " << bornemin[i]
                             << endl;
        } // if !infty
