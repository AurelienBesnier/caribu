...
Polygone::Polygone(char* line, double name, reel* mini, reel* maxi)
{
        ...
        //      fprintf(stderr,"\t=> Comme les sommets 0, 1 et  2  sont
        //      aligne's, \n"); mini[0] =1; maxi[0]=0; correct=false; return;
        //    }
...
int
Polygone::tout_point_inf(reel& position, char& axe)
{
        short i, compris = 1;
        ...
        for (i = 0; i < nb_sommets; i++) {
                ...
                //		cout << "sommet[i][axe]=" << sommet[i][axe] <<
                //endl;
...
int
Polygone::tout_point_sup(reel& position, char& axe)
{
        short i, compris = 1;
...
int
Polygone::nb_in(reel& amin, reel& amax, char& axe)
{
        int nbp = 0, i;
        bool info = false;
        for (i = 0; i < nb_sommets; i++) {
...
Polygone::calc_surface()
{
        ...
        switch (nb_sommets) {
                ...
                case 2:
                        cerr << "Polygone[calc_surface] Irrtum nb_sommet = "
                             << nb_sommets << endl;
                        exit(-1);
                        break;
                case 3: // triangle
                        ...
                        psurf = sommet[0].dist(sommet[1]) *
                                sommet[0].dist(sommet[2]) * sin(psurf) / 2.0;
...
Triangle::calc_surface()
{
        ...
        psurf = sommet[0].dist(sommet[1]) * sommet[0].dist(sommet[2]) *
                sin(psurf) / 2.0;
...
double
signo3D(Point p1, Point p2, Point p3, Point p4)
{
        double suma;
         ...
         double& s1b)
{
        ...
        tmp[3] = p4[2] * (p2[0] * p3[1]) + p3[2] * (p2[1] * p4[0]) +
                 p2[2] * (p3[0] * p4[1]) - p2[2] * (p3[1] * p4[0]) -
                 p3[2] * (p4[1] * p2[0]) - p4[2] * (p2[1] * p3[0]);
...
double
Triangle::intersect(Param_Inter& parag, Point* I)
{
        ...
        if ((a > 0 && b > 0 && c > 0) ||
            (a < 0 && b < 0 && c < 0)) { // Il y a intersection
                // printf("*** h1=%g, h2=%g -->
...
Triangle::Triangle(char* str, double nome, reel* mini, reel* maxi)
  : Polygone(str, nome, mini, maxi)
{
