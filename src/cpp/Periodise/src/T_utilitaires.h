...
class Tabdyn
{
      ...
      public:
        ...
        {
                if (D > 1) {
                        cerr << "Tabdyn[quicksort] implemente que pour les "
                                "tableaux 1D\n";
                        exit(-1);
                }
                if (!trie) {
...
{
        if (tab == NULL) {
                cerr << "\nTabdyn [maj()]: Tableau dynamique pas alloue : "
                        "acces impossible!\n";
                exit(-1);
        }
        unsigned long i;
...
Tabdyn<Type, D>::operator()(int first)
{
        if (D == 1)
                ...
                fprintf(stderr,
                        "Tabdyn<Type,D>::operator()(int first) incompatbile "
                        "avec D=%d\n",
                        D);
                exit(-1);
...
{
        /*  if(tab==NULL) {
          cerr<< "Tabdyn [operator()]: Tableau dynamique pas alloue : acces
        impossible!\n"; exit(-1);
        }
        */
        va_list ptarg;
...
{ // fucke', revu MC9/10/96
        ...
        if (!est_vide()) {
                ...
                cerr << "ERREUR - Impossible de detruire un element car liste "
                        "vide\n";
                cerr.flush();
                // exit (1);
        }
...
Liste<Type>::detruire_droite()
{
        ...
        if (!est_vide()) {
                if (!est_fin()) {
                        ...
                        cout << "ERREUR - Impossible de detruire un element "
                                "car fin de liste\n";
                        cout.flush();
                        exit(1);
                }
        } else {
                cout << "ERREUR - Impossible de detruire un element car liste "
                        "vide\n";
                cout.flush();
                exit(1);
        }
...
ListeD<Type>::detruire_droite()
{
        ...
        if (!est_vide()) {
                if (!est_fin()) {
                        ...
                        cerr << " ListeD[detruire_droite]  Impossible de "
                                "detruire un element car fin de liste\n";
        } else
                cerr << "ListeD[detruire_droite] Impossible de detruire un "
