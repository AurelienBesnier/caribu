...
// g++ -w -O2 -I . -I ../bibliotek -o periodise  periodise.C
// ../binobj/canopy_io.o ../binobj/diffuseur.o  ../binobj/primitive.o
// ../binobj/actop.o ../binobj/outils.o ../binobj/anamatide.o
// ../binobj/T_geometrie.o ../binobj/grille.o ../binobj/boite.o -lm

// Compilation Radia
// g++ -w -O2 -I . -I ../bibliotek -o periodise  periodise.C $EXE/canopy_io.o
// $EXE/diffuseur.o  $EXE/primitive.o $EXE/actop.o $EXE/outils.o
// $EXE/anamatide.o $EXE/T_geometrie.o $EXE/grille.o $EXE/boite.o -lm

/*
#######################################
05/04/05 : ajout de l'option -p pour permettre de specifier
le fichier de proprietes optiques
14/09/09 : gestionde primitives non correctes pour conserver le .can original
(-> pyCaribu)
#######################################
/*/
...
inline void
beep(const char* msg = "M'enfin ...")
{
        cout << (char)7 << msg << endl;
}

int
periodise(int argc, char** argv)
{
        // Gestion des options
        ...
        if (ok < 2) {
                printf(" ==> les options -m  et -8 sont strictement "
                       "necessaires. \n");
                exit(0);
        } // pas good opt
        FILE *fout, *fz;
        ...
        printf(" delta2 = %g\n", delta[2]);
        for (i = 0; i < 3; i++)
                cout << " [1]Bornemin[" << (int)i << "] = " << bmin[i]
                     << "  Bornemax[" << (int)i << "] = " << bmax[i] << endl;
        for (scene.liste_diff_scene.debut(); !scene.liste_diff_scene.finito();
             scene.liste_diff_scene.suivant()) {
                delta[0] = delta[1] = 0;
                pdiff = scene.liste_diff_scene.contenu();
                //    if ( &(pdiff->primi()) == NULL ){// maintient des pas bon
                //    triangles - MC09
                ...
                for (i = 0; i < nbs; i++) {
                        ...
                        // printf(" %d => %g %g %g \n",i,Px,Py,Pz);
                        //  la ligne suivante generait un Bus error => scinde en
                        //  3 lignes et ca marche ??
