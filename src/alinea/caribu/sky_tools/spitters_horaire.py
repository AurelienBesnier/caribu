...
def extra(Rg, DOY, heureTU, latitude):
    ...
    Io = (
        1370 * (1 + 0.033 * cos(2 * pi * (DOY - 4) / 366))
    )  # eclairement (w/m2) a la limitte de l'atmosphere dans un plan perpendiculaire aux rayons du soleil, fonction du jour
    ...
    def __call__(self, Tab_Rg, latitude):
        ...
        for group in range(len(Tab_Rg)):
            ...
            for i in range(1, len(Tab_Rg[group][0])):
                Rg, DOY, heureTU = (
                    float(Tab_Rg[group][2][i]),
                    int(Tab_Rg[group][0][i]),
                    float(Tab_Rg[group][1][i]),
                )
