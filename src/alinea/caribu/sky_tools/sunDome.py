...
def plotSkyTurtle():
    ...
    for i in range(len(elevations)):
        pos = pgl.Vector3(
            pgl.Vector3.Spherical(30, radians(azimuths[i]), radians(90 - elevations[i]))
        )
...
def getSkyTurtleDir(i):
    return -pgl.Vector3(
        pgl.Vector3.Spherical(
            1, radians(azimuths[i - 1]), radians(90 - elevations[i - 1])
        )
    )
...
def plotDirect(
    latitude, longitude, jourJul, startH, stopH, step=30, decalSun=1, decalGMT=0
):
    sunPos = getDirectLight(
        latitude, longitude, jourJul, startH, stopH, step, decalSun, decalGMT
    )
    sc = pgl.Scene()
    for i in range(len(sunPos)):
        # print sunPos[i][0],sunPos[i][1], sunPos[i][2]
        pos = pgl.Vector3(
            pgl.Vector3.Spherical(30, radians(sunPos[i][0]), radians(90 - sunPos[i][1]))
        )
