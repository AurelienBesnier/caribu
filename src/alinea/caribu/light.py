...
def _turtle(sectors="46", format="soc", energy=1.0):
...
def turtle(sectors="46", format="soc", energy=1.0):
    ...
    res = [
        (
            energy,
            emission_inv(energy, elevation),
            (vecteur_direction(elevation, azimuth)),
            elevation,
            azimuth,
        )
        for elevation, azimuth, energy in _turtle(sectors, format, energy)
    ]
    return list(zip(*res))
...
def light_sources(elevation, azimuth, irradiance, orientation=0):
    ...
    return [
        (irr, (xx, yy, zz))
        for irr, xx, yy, zz in zip(array(irradiance), array(x), array(y), array(z))
    ]
