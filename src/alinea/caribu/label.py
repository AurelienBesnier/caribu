...
def encode_label(opt_id=1, opak=0, plant_id=1, elt_id=1, minlength=1):
    ...
    opt_id, opak, plant_id, elt_id = [
        _complete(x, maxlen) for x in [opt_id, opak, plant_id, elt_id]
    ]

    return [
        canlabel_string(opt_id[i], opak[i], plant_id[i], elt_id[i])
...
def decode_label(label):
    ...
    properties = [
        (lab.optical_id, lab.transparency, lab.plant_id, lab.elt_id)
        for lab in (Label(labstring) for labstring in label)
    ]
